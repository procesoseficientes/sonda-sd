...
(function($) {
  ...
  $.floatThead.defaults = {
    ...
    ariaLabel: function($table, $headerCell, columnIndex) {
      // This function will run for every header cell that exists in the table when we add aria-labels.
  ...
  var ieVersion = (function() {
    for (
      var a = 3, b = document.createElement("b"), c = b.all || [];
      (a = 1 + a),
        (b.innerHTML = "<!--[if gt IE " + a + "]><i><![endif]-->"),
        c[0];
  ...
  $.fn.floatThead = function(map) {
    ...
    this.filter(":not(." + opts.floatTableClass + ")").each(function() {
      ...
      var $responsiveContainer = locked
        ? $([])
        : opts.responsiveContainer($table) || $([]);
      ...
      if (haveCaption) {
        var captionAlignTop =
          ($caption.css("caption-side") || $caption.attr("align") || "top") ===
          "top";
      }
      ...
      var absoluteToFixedOnScroll =
        ieVersion <= 9 && !locked && useAbsolutePositioning; //on IE using absolute positioning doesn't look good with window scrolling, so we change position to fixed on scroll, and then change it back to absolute when done.
      ...
      var $fthRow = $("<fthtr>").css({
        //created unstyled elements (used for sizing the table because chrome can't read <col> width)
        display: "table-row",
        "border-spacing": 0,
        height: 0,
        "border-collapse": "collapse"
      });
      var $floatContainer = $("<div>")
        .css(opts.floatContainerCss)
      ...
      $floatTable
        .addClass(opts.floatTableClass)
        .css({ margin: 0, "border-bottom-width": 0 }); //must have no margins or you won't be able to click on things under floating table

      if (useAbsolutePositioning) {
        var makeRelative = function($container, alwaysWrap) {
          var positionCss = $container.css("position");
          var relativeToScrollContainer =
            positionCss === "relative" || positionCss === "absolute";
          ...
          if (!relativeToScrollContainer || alwaysWrap) {
            ...
            $containerWrap =
              ...
              $container
                .wrap(
                  $("<div>")
                    .addClass(opts.floatWrapperClass)
                    .css({
                      position: "relative",
                      clear: "both"
                    })
      ...
      function setHeaderHeight() {
        ...
        if ($table.css("border-collapse") === "collapse") {
          var tableBorderTopHeight = parseInt(
            $table.css("border-top-width"),
            10
          );
          var cellBorderTopHeight = parseInt(
            $table
              .find("thead tr:first")
              .find(">*:first")
      ...
      function setFloatWidth() {
        ...
        var floatContainerWidth =
          $container.css("overflow-y") !== "hidden"
            ? width - scrollbarOffset.vertical
            : width;
      ...
      function updateScrollingOffsets() {
        scrollingTop =
          (util.isFunction(opts.top) ? opts.top($table) : opts.top) || 0;
        scrollingBottom =
          (util.isFunction(opts.bottom) ? opts.bottom($table) : opts.bottom) ||
          0;
      }
      ...
      function columnNum() {
        ...
        if (count !== lastColumnCount) {
          ...
          for (var x = 0; x < count; x++) {
            ...
            cell.setAttribute(
              "aria-label",
              opts.ariaLabel($table, $headerColumns.eq(x), x)
            );
      ...
      function calculateFloatContainerPosFn() {
        ...
        var captionScrollOffset = captionAlignTop
          ? captionHeight
          : -captionHeight;
        ...
        if (locked) {
          ...
          tableContainerGap =
            tableOffset.top - containerOffset.top + scrollingContainerTop;
          ...
          floatEnd =
            tableOffset.top -
            scrollingTop -
            floatContainerHeight +
            scrollingBottom +
            scrollbarOffset.horizontal;
        }
        var windowTop = $window.scrollTop();
        var windowLeft = $window.scrollLeft();
        var getScrollContainerLeft = function() {
          return (
            (isResponsiveContainerActive()
              ? $responsiveContainer
              : $scrollContainer
            ).scrollLeft() || 0
          );
        ...
        return function(eventType) {
          ...
          var isTableHidden =
            $table[0].offsetWidth <= 0 && $table[0].offsetHeight <= 0;
          ...
          if (locked && useAbsolutePositioning) {
            ...
            if (tableContainerGap >= scrollingContainerTop) {
              ...
              scrollingContainerTop - tableContainerGap >
              tableHeight - floatContainerHeight
            ) {
            ...
            if (windowTop > floatEnd + tableHeight + captionScrollOffset) {
              top =
                ...
                floatContainerHeight +
                captionScrollOffset +
                scrollingBottom; //scrolled past table
              ...
              top =
                ...
                tableOffset.top +
                tableContainerGap +
                (captionAlignTop ? captionHeight : 0);
            ...
            if (
              tableContainerGap > scrollingContainerTop ||
        ...
        return function(pos, setWidth, setHeight) {
          if (pos != null && (oldTop !== pos.top || oldLeft !== pos.left)) {
            if (ieVersion === 8) {
              ...
              var transform =
                "translateX(" + pos.left + "px) translateY(" + pos.top + "px)";
              var cssObj = {
      ...
      function calculateScrollBarSize() {
        ...
        if ($scrollContainer.length) {
          if (
            opts.support &&
            opts.support.perfectScrollbar &&
            $scrollContainer.data().perfectScrollbar
          ) {
            ...
            if ($scrollContainer.css("overflow-x") === "scroll") {
              ...
              scrollbarOffset.horizontal =
                sw - offsetv < tw ? scrollbarWidth : 0;
            }
      ...
      var windowScrollDoneEvent = util.debounce(function() {
        repositionFloatContainer(
          calculateFloatContainerPos("windowScrollDone"),
          false
        );
      }, 1);
      ...
      var windowScrollEvent = function() {
        repositionFloatContainer(
          calculateFloatContainerPos("windowScroll"),
          false
        );
        if (absoluteToFixedOnScroll) {
      ...
      var containerScrollEvent = function() {
        repositionFloatContainer(
          calculateFloatContainerPos("containerScroll"),
          false
        );
      };
      ...
      var windowResizeEvent = function() {
        ...
        repositionFloatContainer(
          calculateFloatContainerPos("resize"),
          true,
          true
        );
      };
      var reflowEvent = util.debounce(function() {
        ...
        calculateFloatContainerPos = calculateFloatContainerPosFn();
        repositionFloatContainer(
          calculateFloatContainerPos("reflow"),
          true,
          true
        );
      }, 1);
      ...
      if (canObserveMutations) {
        ...
        if (!mutationElement) {
          mutationElement = $scrollContainer.length
            ? $scrollContainer[0]
            : $table[0];
        }
        mObs = new MutationObserver(function(e) {
          ...
          for (var i = 0; i < e.length; i++) {
            if (
              !(
                wasTableRelated(e[i].addedNodes) ||
      ...
      $table.data("floatThead-attached", {
        ...
        getRowGroups: function() {
          if (headerFloated) {
            return $floatContainer
              .find(">table>thead")
              .add($table.children("tbody,tfoot"));
          } else {
