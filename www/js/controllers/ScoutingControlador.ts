class ScoutingControlador {
    ...
    delegarScoutingControlador() {
        ...
        $("#UiScoutingPage").on("pageshow", () => {
            ...
            este.limpiarCamposDeScouting(() => {
                este.reglaServicio.obtenerRegla(ReglaTipo.Scouting.toString(), (reglasDeScouting) => {
        ...
        $("#UiBtnTakePicture1Scouting").on("click", () => {
            DispositivoServicio.TomarFoto((imgUrl) => {
                let imgOneScouting = $("#UiImg1Scouting");
        ...
        $("#UiBtnTakePicture2Scouting").on("click", () => {
            DispositivoServicio.TomarFoto((imgUrl) => {
                let imgTwoScouting = $("#UiImg2Scouting");
        ...
        $("#UiBtnTakePicture3Scouting").on("click", () => {
            DispositivoServicio.TomarFoto((imgUrl) => {
                let imgThreeScouting = $("#UiImg3Scouting");
    ...
    limpiarCamposDeScouting(callback:()=>void): void {
        let nameScouting = $("#UiTxtNameScouting");
        let taxidScouting = $("#UiTxtTaxIdScouting");
        let directionScouting = $("#UiTxtDirectionScouting");
        let nameContactScouting = $("#UiTxtNameContactScouting");
        let telephoneContactScouting = $("#UiTxtTelephoneContactScouting");
        let invoiceNameScouting = $("#UiTxtTaxInviceName");
        let invoiceAddressScouting = $("#UiTxtTaxAddress");
        let imgOneScouting = $("#UiImg1Scouting");
        let imgTwoScouting = $("#UiImg2Scouting");
        let imgThreeScouting = $("#UiImg3Scouting");
        let uiLabelTaxId = $("#UiLabelTaxId");
        let etiquetaDeImpuesto = localStorage.getItem("TAX_ID");
    ...
    recolectarInformacionBasicaDeNuevoCliente(callback:(nuevoCliente: Cliente)=>void, errorCallback: (resultado: Operacion)=>void): void {
        let _this = this;
        try {
            PagoConsignacionesServicio
                .ValidarSequenciaDeDocumentos(SecuenciaDeDocumentoTipo.Scouting,
                    (isValidSequence) => {
                        if (isValidSequence) {
                            PagoConsignacionesServicio
                                .ObtenerSiguienteSecuenciaDeDocumento(SecuenciaDeDocumentoTipo.Scouting,
                                    (docSerie, docNum) => {
                                        GetNexSequence("SCOUTING",
                                            (seq: string) => {
                                                let nameClient = $("#UiTxtNameScouting");
                                                let taxIdClient = $("#UiTxtTaxIdScouting");
                                                let addressClient = $("#UiTxtDirectionScouting");
                                                let contactNameClient = $("#UiTxtNameContactScouting");
                                                let telephoneContactNameClient = $("#UiTxtTelephoneContactScouting");
                                                let invoiceNameClient = $("#UiTxtTaxInviceName");
                                                let invoiceAddressClient = $("#UiTxtTaxAddress");
                                                let imgOneClient = $("#UiImg1Scouting");
                                                let imgTwoClient = $("#UiImg2Scouting");
                                                let imgThreeClient = $("#UiImg3Scouting");
                                                ...
                                                if (nameClient.val() === "") {
                                                    ...
                                                    errorCallback({ codigo: -1, resultado: ResultadoOperacionTipo.Error, mensaje: "Debe proporcionar el nombre del cliente." } as Operacion);
                                                    return;
                                                } else {
                                    ...
                                    (error) => {
                                        errorCallback({ codigo: -1, resultado: ResultadoOperacionTipo.Error, mensaje: error } as Operacion);
                                        return;
                                    });
                        } else {
                            errorCallback({ codigo: -1, resultado: ResultadoOperacionTipo.Error, mensaje: "No tiene una secuencia valida para crear el nuevo cliente, por favor, comuníquese con su Administrador." } as Operacion);
                            return;
                        }
                    },
                    (error) => {
    ...
    recolectarInformacionDeEtiquetasDeNuevoCliente(cliente: Cliente,
        callback: (clienteCompleto: Cliente) => void,
        errorCallback: (resultado: Operacion) => void): void {
        try {
            let contenedorDeEtiquetas = $('input[type=checkbox]:checked');
            let etiquetasDeCliente: Etiqueta[] = new Array<Etiqueta>();
            if (contenedorDeEtiquetas) {
                etiquetasDeCliente = contenedorDeEtiquetas.map((index: number, element: any) => {
                    let control = element;
                    return (<Etiqueta>{ tagColor: $(control).attr("id"), docSerieClient: cliente.docSerie, docNumClient: cliente.docNum});
                }).get();
    ...
    recogerDatosDeNuevoClienteYGuardarlo(este: ScoutingControlador) {
        este.recolectarInformacionBasicaDeNuevoCliente((clienteTemp) => {
                este.recolectarInformacionDeEtiquetasDeNuevoCliente(clienteTemp,
                    (clienteCompleto) => {
                        este.validarReglasDeScouting(clienteCompleto,
                            este.reglasDeScouting,
                            0,
                            (clienteRetornado) => {
                                este.clienteServicio.guardarScouting(clienteRetornado,
    ...
    validarReglasDeScouting(cliente: Cliente,reglasDeScouting, indiceDeReglaActual, callback:(cliente: Cliente)=>void, errorCallback:(resultado: Operacion)=>void): void {
        let este = this;
        try {
            ...
            if (indiceDeReglaActual < reglasDeScouting.rows.length) {
                ...
                switch (reglaAct.TYPE_ACTION) {
                    case "FotografiaObligatoria":
                        ...
                        break;
                    default:
                        este.validarReglasDeScouting(cliente,
                            reglasDeScouting,
    ...
    cargarEtiquetasParaNuevoCliente(callback: () => void, errorCallback: (resultado: any) => void) {
        let este = this;
        try {
            este.etiquetaServicio.obtenerEtiquetas((etiquetas) => {
                let li: string = "";
                let contenedorEtiquetas = $("#UiListTagsForScouting");
