      ...
      if (this.cliente.totalAmout <= 0) {
        ...
        this.validarConfiguracionDeBonificacionPorCombos(
          () => {
            ...
            let uiNumeroDeOrdenDeCompraDeOrdenDeVenta = $("#UiNumeroDeOrdenDeCompraDeOrdenDeVenta");
            this.cliente.purchaseOrderNumber = uiNumeroDeOrdenDeCompraDeOrdenDeVenta.val();
            this.cliente.purchaseOrderNumber.trim();
  ...
  finalizarOrdenDeVenta() {
    try {
      navigator.notification.confirm(
        ...
        buttonIndex => {
          if (buttonIndex === 2) {
            let minimumAmount = parseInt(
              localStorage.getItem("MINIMUM_ORDER_AMOUNT")
            );
            let saleTotal = this.obtenerTotalDeOrdenDeVenta(
              this.cliente.appliedDiscount,
              this.listaDeSkuOrdenDeVenta
            );
            if (saleTotal >= minimumAmount) {
              ...
              notify(
                `No puede realizar venta porque no alcanza el monto mínimo: ${minimumAmount}`
              );
            }
          }
        },
