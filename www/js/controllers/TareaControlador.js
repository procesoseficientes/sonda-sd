var TareaControlador = (function () {
    ...
    TareaControlador.prototype.delegarTareaControlador = function () {
        ...
        $("#UiBtnActualizarPlanDeRuta").on("click", function () {
            ...
            TareaServicio.recalcularSecuenciaDeTareas(function () {
                ...
                if (gIsOnline === SiNo.No) {
                    mensaje += " No se actualizará el plan de ruta en el servidor por falta de conexión";
                }
                notify(mensaje);
                EnviarData();
                _this.UsuarioDeseaObtenerTareasPorEstado("'" + TareaEstado.Asignada + "','" + TareaEstado.Aceptada + "'", function (listaTareas) {
        ...
        $("#UiBtnScanRgaCode").on("click", function () {
            ...
            _this.UsuarioDeseaEscanearCodigoRga(function (codigoRga) {
                window.estaEnEscaneoDeRga = false;
                console.log(codigoRga);
                _this.UsuarioDeseaObtenerTareasPorCodigoRga(codigoRga, function (listaTareas) {
                    if (listaTareas.length === 1) {
                        _this.CrearListadoDeTareas(listaTareas, function () {
                            codigoRgaTxt.val("");
                            codigoRgaTxt = null;
                            if (_this.EsTareaDeVenta(listaTareas[0])) {
                                InvoiceThisTask(listaTareas[0].TASK_ID, listaTareas[0].RELATED_CLIENT_CODE, listaTareas[0].RELATED_CLIENT_NAME, listaTareas[0].NIT, listaTareas[0].TASK_TYPE);
                            }
                        });
        ...
        $("#UiBtnMostrarTareasCompletadas").on("click", function () {
            _this.UsuarioDeseaObtenerTareasPorEstado("'" + TareaEstado.Completada + "'", function (listaTareas) {
                _this.CrearListadoDeTareas(listaTareas, function () {
                    //..
                });
            }, function (error) {
        ...
        $("#uiTxtFiltroClientes").on("keyup", function (e) {
            if (e.keyCode === 13) {
                ...
                if (codigoRga.val() === "" || codigoRga.val() === " ") {
                    ...
                    _this.UsuarioDeseaObtenerTareasPorCodigoRga(codigoRga.val(), function (listaTareas) {
                        if (listaTareas.length === 1) {
                            _this.CrearListadoDeTareas(listaTareas, function () {
                                codigoRga.val("");
                                if (_this.EsTareaDeVenta(listaTareas[0])) {
    ...
    TareaControlador.prototype.CrearListadoDeTareas = function (listaTareas, callBack) {
        try {
            var este = this;
            var objetoListaTareas = $("#skus_listview_sales_route");
            objetoListaTareas.children().remove("li");

            var listadoDeTareasDeVenta = listaTareas
                .filter(function (tareaVenta) {
                    // ReSharper disable once CoercedEqualsUsing
            ...
            for (var i = 0; i < listadoDeTareasDeVenta.length; i++) {
                ...
                if (!este.VerificarSiTraeGps(tarea)) {
                    ...
                    li += '<li data-mini="true" class="ui-alt-icon ui-nodisc-icon ui-shadow ui-icon-forbidden">';
                } else {
                    xonclick1 = "TaskNavigateTo('" + tarea.EXPECTED_GPS + "',null);";
                    li += '<li data-mini="true" class="ui-alt-icon ui-nodisc-icon ui-shadow ui-icon-navigation">';
                }

                var xonclick2 = "gTaskOnRoutePlan = 1; InvoiceThisTask(" +
                    tarea.TASK_ID +
                    ",'" +
                ...
                li += '<a href="#" onclick="' + xonclick2 + '" id="' + "TA_" + tarea.TASK_ID + '">';
                li += "<h2>";
                li += '<span class="small-roboto">';
                li += (i + 1) + ")</span>&nbsp";
                li += '<span class="small-roboto">' + tarea.RELATED_CLIENT_CODE + "</span><br>";
                li += '<span class="small-roboto">' + tarea.RELATED_CLIENT_NAME + "</span>";
                li += "</h2>";
                li += "<p>" + tarea.TASK_ADDRESS + "</p>";
                li += "<p style='display: none'>" + tarea.RGA_CODE + "</p>";
                if (tarea.BUY_TYPE === "Crédito") {
                    li += "<p>" + tarea.BUY_TYPE + ": " + window.accounting.formatMoney((tarea.CURRENT_BALANCE - tarea.CREDIT_AMOUNT)) + "</p>";
                } else {
            ...
            for (var j = 0; j < listadoDeTareasDeEntrega.length; j++) {
                ...
                var deliveryTaskClick = "gTaskOnRoutePlan = 1; InvoiceThisTask(" +
                ...
                li += '<a href="#" onclick="' + deliveryTaskClick + '" id="' + "TA_" + tareaEntrega.TASK_ID + '">';
                li += "<h2>";
                li += '<span class="small-roboto">';
                li += (j + 1) + ")</span>&nbsp";
                li += '<span class="small-roboto">' + tareaEntrega.RELATED_CLIENT_CODE + "</span><br>";
                li += '<span class="small-roboto">' + tareaEntrega.RELATED_CLIENT_NAME + "</span>";
                li += "</h2>";
            ...
            if (li !== "") { //TODO: Validar si es necesario el refresh.
                objetoListaTareas.append(li);
                objetoListaTareas.listview("refresh");
            }
            li = null;
            objetoListaTareas = null;
            document.getElementById("lblClientsToVisit").innerText = "Plan de ruta (" + este.ObtenerCantidadTotalDeTareas(listadoDeTareasDeEntrega, listadoDeTareasDeVenta) + ")";
            callBack();
        } catch (e) {
            InteraccionConUsuarioServicio.desbloquearPantalla();
    ...
    TareaControlador.prototype.ObtenerCantidadTotalDeTareas = function (tareasDeVenta, tareasDeEntrega) {
        return tareasDeVenta.length + tareasDeEntrega.length;
    }

    TareaControlador.prototype.UsuarioDeseaObtenerTareasPorEstado = function (status, callBack, errorCallBack) {
    ...
    TareaControlador.prototype.UsuarioDeseaEscanearCodigoRga = function (callBack, returnCallBack, errorCallBack) {
        try {
            cordova.plugins.barcodeScanner.scan(
                ...
                function (error) {
                    console.log("Error al intentar leer el codigo RGA del cliente debido a: " + error);
                    errorCallBack("Error al intentar leer el codigo RGA del cliente debido a: " + error);
                });
        } catch (e) {
            console.log("Error al intentar leer el codigo RGA del cliente debido a: " + e.message);
            errorCallBack("Error al intentar leer el codigo RGA del cliente debido a: " + e.message);
        }
    }

    TareaControlador.prototype.UsuarioDeseaObtenerTareasPorCodigoRga = function (codigoRga, callBack, errorCallBack) {
