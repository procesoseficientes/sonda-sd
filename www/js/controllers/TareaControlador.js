...
function DelegarTareaControlador() {
        ...
        cordova.plugins.diagnostic.isCameraAuthorized(function (enabled) {
            if (enabled) {
                LeerCodigoBarraConCamara(function (codigoLeido) {
                    var uiTxtFiltroClientesAceptadas = $("#uiTxtFiltroClientesAsignados");
    ...
    $("#UiBotonCamaraClientesAceptadas").on("click", function () {
        cordova.plugins.diagnostic.isCameraAuthorized(function (enabled) {
            if (enabled) {
                LeerCodigoBarraConCamara(function (codigoLeido) {
                    var uiTxtFiltroClientesAceptadas = $("#uiTxtFiltroClientesAceptadas");
                    uiTxtFiltroClientesAceptadas.val(codigoLeido);
...
function FiltrarClienteCompletadas() {
    ...
    for (var i = 0; i < listadoDeListasCompletadas.length; i++) {
        $("#" + listadoDeListasCompletadas[i] + " li").each(function (e, object, res) {
            var control = $("#" + object.id);
            if (control.attr("rga") === criterioDeFiltro) {
                tareasEcontradas++;
                idTarea = object.id.substring(8, object.id.length);
    ...
    try {
        ...
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                if (tipo === "ALL") {
                    strSql += " SELECT *" +
                        ", (SELECT COUNT(TASK_ID) FROM PRESALES_ROUTE PD WHERE PD.TASK_STATUS = '" + estado + "') AS QTY " +
                        ...
                        "WHERE PP.TASK_STATUS = '" + estado + "' ";
                    strSql += " ORDER BY PP.TASK_SEQ";
                    console.log(strSql);
                } else {
                    strSql += " SELECT *,";
                    strSql += " (SELECT COUNT(PD." + tipo + ") FROM PRESALES_ROUTE PD WHERE PD." + tipo + " = PP." + tipo + " AND PD.TASK_STATUS = '" + estado + "') AS QTY";
                    strSql += " FROM PRESALES_ROUTE PP " +
                        "LEFT JOIN TASK_AUX TU ON (PP.TASK_ID = TU.PRESALES_ROUTE_ID) " +
                        "LEFT JOIN CLIENTS C ON (PP.RELATED_CLIENT_CODE = C.CLIENT_ID) " +
                        "WHERE PP.TASK_STATUS = '" + estado + "' ";
                ...
                tx.executeSql(strSql, [],
                    function (tx, results) {
                        ...
                        for (var i = 0; i <= (results.rows.length - 1) ; i++) {
                            ...
                            switch (tipo) {
                                case 'TASK_TYPE':
                                    if (codigoAcordionPorTipoTarea !== results.rows.item(i).TASK_TYPE) {
                                        codigoAcordionPorTipoTarea = results.rows.item(i).TASK_TYPE;
                                        crearEtiqueta = true;
                                    }
                                    break;
                                case 'RELATED_CLIENT_CODE':
                                    if (codigoAcordionPorTipoTarea !== results.rows.item(i).RELATED_CLIENT_CODE) {
                                        codigoAcordionPorTipoTarea = results.rows.item(i).RELATED_CLIENT_CODE;
                                        crearEtiqueta = true;
                                    }
                                    break;
                                case 'ALL':
                            ...
                            if (crearEtiqueta) {
                                var acordion = obtenerAcordionParaListaTarea(codigoAcordionPorTipoTarea, estado, results.rows.item(i).QTY, results.rows.item(i).RELATED_CLIENT_NAME);

                                pAcordionElement.append(acordion).collapsibleset('refresh');
                                pAcordionElement.trigger('create');
                            }

                            vLi = obtenerLiParaListaDeTareas(results.rows.item(i).TASK_ID, results.rows.item(i).TASK_TYPE, results.rows.item(i).TASK_STATUS, results.rows.item(i).RELATED_CLIENT_CODE, results.rows.item(i).RELATED_CLIENT_NAME, results.rows.item(i).TASK_ADDRESS, results.rows.item(i).NO_PICKEDUP, i, results.rows.item(i).RGA_CODE);

                            var uilista = $("#Lista" + estado + "-" + codigoAcordionPorTipoTarea);
                            uilista.append(vLi);
                            uilista.listview('refresh');


                            var uiCantidadAcordion = $("#Cant" + estado + "-" + codigoAcordionPorTipoTarea);
                            uiCantidadAcordion.text(parseInt(uilista["0"].childElementCount));


                            uilista = null;
                            if (i === 0) {
                                var uiAcordion = $("#Acordion" + estado + "-" + codigoAcordionPorTipoTarea);
                        ...
                        if (contadorListas === 3) {
                            ...
                            if (window.vistaCargandosePorPrimeraVez) {
                                notify("La información de la Ruta ha sido cargada exitosamente...");
                            }
                        }
                    },
                    function (err) {
...
function obtenerLiParaListaDeTareas(idTarea, tipoDeTarea, estadoTarea, codigoCliente, nombreCliente, direccionCliente, noReolectado, indiceDeLista, codeRga) {
    var vLi = "";
    var totales = "";
    var pClick = "gettask(" + idTarea + ");";
    vLi += '';
    vLi += '<li style="opacity: 1" class="ui-alt-icon ui-nodisc-icon" id="LITAREA-' + idTarea + '" rga="' + codeRga + '"> ' +
        '<a href="#" onclick=' + pClick + ' id="TAREA-' + idTarea + '" >';

    vLi += '<p><span class="small-roboto">' + (indiceDeLista + 1) + ') </span> <span class="small-roboto" id="UiEtiquetaTskCliente-' + idTarea + '">' + codigoCliente + ' ' + nombreCliente + '</span></p>';
    vLi += '<p><span class="small-roboto" id="UiEtiquetaTskClienteDireccion-' + idTarea + '">' + direccionCliente + '</span></p>';
    ...
    switch (estadoTarea) {
        ...
        case TareaEstado.Completada:
            ...
            break;
        default:
            vLi += '<span class="ui-li-count ui-btn small-roboto" style="background-color:silver">Sin Status ' + estadoTarea + '</span>';
            break;
    };

    switch (noReolectado) {
        case 1:
            vLi += '<p><span class="small-roboto" style="text-shadow: none; color:#ffffff; background-color:orangered">No Recolectado. ' + noReolectado + '</span></p>';
            break;
        case 0:
...
function obtenerAcordionParaListaTarea(codigoAcordionPorTipoTarea, estadoDeTarea, cantidad, nombreCliente) {
    var nombreAcordion = "";
    ...
    var nombreLista = "Lista" + estadoDeTarea + "-" + codigoAcordionPorTipoTarea + "";
    switch (estadoDeTarea) {
        case "ASSIGNED":
    ...
    var acordion = "";
    acordion += "<div data-role='collapsible' id='Acordion" + estadoDeTarea + "-" + codigoAcordionPorTipoTarea + "'>";
    acordion += "<h5>" + nombreAcordion + "<span class='ui-li-count' id='Cant" + estadoDeTarea + "-" + codigoAcordionPorTipoTarea + "'>" + cantidad + "</span></h5>";
    acordion += "<ul data-role='listview' data-mini='true' data-filter='true' data-input='#" + obtenerNombreFiltro(estadoDeTarea) + "' data-filter-placeholder='Buscar' data-filter-theme='a' data-divider-theme='c' id='Lista" + estadoDeTarea + "-" + codigoAcordionPorTipoTarea + "' ></ul>";
    acordion += "</div>";
...
function actualizarListadoDeTareas(idTarea, tipoDeTarea, estadoDeTarea, codigoCliente, nombreCliente, direccionCliente, noReolectado, estadoDeTareaAnterior, rgaCode) {
    ...
    if (uiAcordionPorTipo === undefined || uiAcordionPorTipo === null || uiAcordionPorTipo.length === 0) {
        var acordion = obtenerAcordionParaListaTarea(tipoDeTarea, estadoDeTarea, 1, nombreCliente);
        pAcordionElement.append(acordion).collapsibleset('refresh');
        pAcordionElement.trigger('create');
    } else {
    ...
    if (estadoDeTarea !== estadoDeTareaAnterior) {
        ...
        var li = obtenerLiParaListaDeTareas(idTarea, tipoDeTarea, estadoDeTarea, codigoCliente, nombreCliente, direccionCliente, noReolectado, parseInt(cantidadAcordion), rgaCode);
        uiLista.append(li);
        uiLista.listview('refresh');
...
function actualizarDatosDeTareaDom(tareaId, idCliente, nombreCliente, direccionCliente) {
    var uiEtiquetaTskCliente = $("#UiEtiquetaTskCliente-" + tareaId);
    var uiEtiquetaTskClienteDireccion = $("#UiEtiquetaTskClienteDireccion-" + tareaId);
