...
function delegarTareaFueraDeRutaControlador() {
  $("#UiBtnVolverAMenuNuevaTarea").on("click", function() {
  ...
  $("#uiBtnLimpiarFiltroClientesNuevaTarea").on("click", function() {
    limpiarFiltro();
  });

  $("#UiBotonCamaraClientesNuevaTarea").on("click", function() {
  ...
  $("#uiTxtFiltroClientesNuevaTarea").on("keypress", function(e) {
    if (e.keyCode === 13) {
      e.preventDefault();
  ...
  $("#UiNewTaskOutsideOfRoutePlanPage").on(
    "click",
...
function delegarSocketsTareaFueraDeRutaControlador(socketIo) {
  socketIo.on("GetClientsForTaskOutsideTheRoutePlan_Response", function(data) {
    cargarClientes(data);
  });

  socketIo.on("CreateTaskOutsideTheRoutePlan_Response", function(data) {
    if (data.data && data.data.tarea) {
      var tarea = data.data.tarea;
      var estadistica = data.data.estadisticaPorCliente;
      insertarTareaFueraDeRuta(data, function(data) {
        insertarListaDePreciosPorClienteFueraDeRuta(
          data,
          data.data.cliente,
          function(data) {
            insertarListaDePreciosPorSkuFueraDeRuta(data, function(data) {
              insertarConsignaciones(
                data.data,
                data.data.consignaciones,
                data.data.detalleconsignaciones,
                function(originalData) {
                  agregarInformacionDeFacturasVencidas(originalData, function(
                    dataProcess
                  ) {
                    agregarInformacionDeCuentaCorriente(
                      dataProcess,
                      function() {
                        var estadisticaDeVentaServicio = new EstadisticaDeVentaServicio();
                        estadisticaDeVentaServicio.agregarEstadisticaDeVentaPorClienteFueraDeRuta(
                          estadistica,
                          function() {
                            gTaskOnRoutePlan = 0;
                            InvoiceThisTask(
      ...
      console.log(
        "Error al procesar la tarea fuera del plan de ruta en el servidor, fallo: "
      );
      console.dir(data);
      notify(
        "Ha ocurrido un error al procesar la tarea fuera del plan de ruta, por favor, vuelva a intentar."
      );
    }
  });
}
...
function limpiarFiltro() {
  try {
    ...
    document.getElementById(
      "UiContenedorListaDeClientesNuevaTarea"
    ).style.display = "none";
    document.getElementById("UiNotificacionClientesNuevaTarea").style.display =
      "none";
  } catch (e) {
...
function cargarClientes(data) {
  try {
    ...
    if (data.clientes.length > 0) {
      ...
      document.getElementById(
        "UiNotificacionClientesNuevaTarea"
      ).style.display = "none";
    } else {
      document.getElementById(
        "UiNotificacionClientesNuevaTarea"
      ...
      notify(
        "No se han encontrado coincidencias para el criterio proporcionado, por favor verifique y vuelva a intentar."
      );
    }
    ...
    for (var i = 0, a = data.clientes; i < a.length; i++) {
      ...
      var li = "";
      if (
        clienteTemp.ADRESS_CUSTOMER == null ||
        clienteTemp.ADRESS_CUSTOMER === "null"
      )
        clienteTemp.ADRESS_CUSTOMER = "No tiene direccion";
      var direccion = "";
      ...
      li =
        "<li id=" +
        clienteTemp.CODE_CUSTOMER +
        " class='small-roboto ui-content'>\n                        <a href='#'>\n                        <p>\n                        <span><strong>CLIENTE: </strong>" +
        clienteTemp.CODE_CUSTOMER +
        "</span>\t\t                        <br><span><strong>" +
        etiquetaDeImpuesto +
        ": </strong>" +
    ...
    my_dialog("", "", "close");
    notify(
      "No se ha podido cargar la lista de clientes debido a: " + e.message
    );
  }
}
