class ControlDeFinDeRutaControlador {
  ...
  delegarControlDeFinDeRuta() {
    ...
    $("#BotonSincronizarTodosLosDocumentosPendientesDeEnvio").on(
      "click",
      () => {
  ...
  generarListadoDeDocumentos(documentosDeFinDeRuta) {
    let contenedorDeTablaDeInformacionDeFinDeRuta = $(
    ...
    try {
      ...
      //Encabezado de tabla
      htmlDeTablaDeInformacionDeFinDeRuta.push(`<table id="ContenedorDeInformacionDeFinDeRuta" class="table-stroke">
                                                      <thead>
                                                        <tr>
                                                          <th>No. Documento</th>
                                                          <th>Tipo</th>
      ...
      if (agregarTablaDeInformacion) {
        contenedorDeTablaDeInformacionDeFinDeRuta.append(
          htmlDeTablaDeInformacionDeFinDeRuta.join("")
        );
        this.generarInformacionDePedidosParaResumenDeInformacionDeFinDeRuta(
          documentosDeFinDeRuta
        );
      } else {
        contenedorDeTablaDeInformacionDeFinDeRuta.append(
          `<p id="EtiquetaDeNotificacionDeFaltaDeDocumentosParaFinDeRuta">No se encontraron documentos para el fin de ruta </p>`
        );
      }
    } catch (e) {
      notify(
        `No se ha podido mostrar la información de los documentos debido a: ${e.message}`
      );
    } finally {
  ...
  generarInformacionDePedidosParaResumenDeInformacionDeFinDeRuta(
    documentosDeFinDeRuta
  ) {
    let cantidadTotalDeDocumentosDeTipoPedidoDeRuta = $(
      "#CantidadTotalDeDocumentosDeTipoPedidoDeRuta"
    );
    let cantidadTotalDeDocumentosDeRutaPendientesDeEnvio = $(
      "#CantidadTotalDeDocumentosDeRutaPendientesDeEnvio"
    );
    try {
      this.obtenerCantidadTotalDeDocumentosPendientesDeSincronizar(
        documentosDeFinDeRuta,
        (documentosNoSincronizados, cantidadTotalDeDocumentos) => {
          cantidadTotalDeDocumentosDeTipoPedidoDeRuta.text(
            cantidadTotalDeDocumentos.toString()
          );
          cantidadTotalDeDocumentosDeRutaPendientesDeEnvio.text(
            documentosNoSincronizados.toString()
          );
          this.habilitarBotonDeEnvioDeTodosLosDocumentosPendientesDeSincronizar(
            documentosNoSincronizados > 0
          );
        }
      );
    } catch (e) {
      notify(
        `No se ha podido generar la información del resumen de pedidos debido a: ${e.message}`
      );
    } finally {
  ...
  enviarTodosLosDocumentosPendientesDeEnvioHaciaElServidor() {
    ...
    try {
      if (gIsOnline === SiNo.Si) {
        ...
        my_dialog(
          "Por favor espere...",
          "Un minuto por favor, estamos sincronizando la información.",
          "open"
        );
        EnviarData();
        ...
        notify(
          "Debe tener conexión al servidor para poder enviar los documentos pendientes de envío."
        );
      }
    } catch (e) {
      DesBloquearPantalla();
      my_dialog("", "", "close");
      notify(
        `No se ha podido enviar los documentos hacia el servidor debido a: ${e.message}`
      );
    }
  }

  permitirFinalizarRuta(
    documentosDeFinDeRuta,
    callback?: (documentosDeFinDeRuta: any) => void
  ) {
    try {
      this.obtenerCantidadTotalDeDocumentosPendientesDeSincronizar(
        documentosDeFinDeRuta,
        (
          cantidadDeDocumentosPendientesDeSincronizar,
          cantidadTotalDeDocumentos
        ) => {
          let botonDeFinalizacionDeRuta = $(
            "#BotonFinalizarRutaYMostrarResumenDeFinDeRuta"
          );
  ...
  habilitarBotonDeEnvioDeTodosLosDocumentosPendientesDeSincronizar(
    habilitarBoton: boolean
  ) {
    let botonSincronizarTodosLosDocumentosPendientesDeEnvio = $(
      "#BotonSincronizarTodosLosDocumentosPendientesDeEnvio"
    );

    if (habilitarBoton) {
      botonSincronizarTodosLosDocumentosPendientesDeEnvio.removeClass(
        "ui-disabled"
      );
      botonSincronizarTodosLosDocumentosPendientesDeEnvio = null;
    } else {
      botonSincronizarTodosLosDocumentosPendientesDeEnvio.addClass(
        "ui-disabled"
      );
      botonSincronizarTodosLosDocumentosPendientesDeEnvio = null;
    }
  ...
  obtenerCantidadTotalDeDocumentosPendientesDeSincronizar(
    documentosDeFinDeRuta: any,
    callback: (
      cantidadDeDocumentosPendientesDeEnvio: number,
      cantidadTotalDeDocumentos: number
    ) => void
  ) {
