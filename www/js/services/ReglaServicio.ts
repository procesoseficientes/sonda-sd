class ReglaServicio implements IReglaServicio {
  guardarReglas(
    ...
    try {
      SONDA_DB_Session.transaction(
        ...
        err => {
          //Si surge algun error al intentar insertar
          errorCallBack(err);
        },
        () => {
  ...
  obtenerRegla(
    tipoRegla: string,
    callbak: (results: SqlResultSet) => void,
      ...
      SONDA_DB_Session.transaction(
        tx => {
          ...
          tx.executeSql(
            ...
            (txResult, err) => {
              if (err.code !== 0) errorCallback(err.message);
            }
          );
        },
        err => {
  ...
  obtenerReglasParaInicioDeTarea(
    callback: (results: SqlResultSet) => void,
    errorCallback: (error: string) => void
  ): void {
    try {
      SONDA_DB_Session.transaction(
        ...
        switch (reglaAct.TYPE) {
                ...
                regla => {
                  if (regla.rows.length > 0) {
                    if (
                      ...
                      this.ejecutarReglasDeInicioDeRuta(
                        reglas,
                        reglaActual + 1,
                        () => {
                          callback();
          ...
          case "VisualizacionDeEstadisticaDeVentaDelCliente":
            if (gClientID != "C000000") {
              let estadisticaDeVentaPorDiaServicio = new EstadisticaDeVentaServicio();

              estadisticaDeVentaPorDiaServicio.obtenerEstadisticaDeVentaPorCliente(
                gClientID,
                (estadisticas: Array<EstadisticaDeVentaPorCliente>) => {
                  if (estadisticas && estadisticas.length > 0) {
                    estadisticaDeVentaPorDiaControlador.funcionDeRetornoAProcesoPrincipal = () => {
                      this.ejecutarReglasDeInicioDeRuta(
                        reglas,
                        reglaActual + 1,
                        () => {
                    ...
                    estadisticaDeVentaPorDiaControlador.construirVisualizacionDeEstadisticaDeCliente(
                      estadisticas,
                      () => {
