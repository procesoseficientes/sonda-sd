class FacturaServicio implements IFacturaServicio {
      ...
      (transaction: SqlTransaction) => {
        transaction.executeSql(
          ...
          (_transaccionRetorno: SqlTransaction, resultados: SqlResultSet) => {
            ...
            if (resultsSize > 0) {
              let facturaDetallesTemporal = resultados.rows;
              let facturaDetalles = new Array();
  ...
  InsertarFactura(
    ...
    sql.push(`) VALUES (`);
    sql.push(
      `${invoice.invoiceNum}, '${invoice.terms}', '${invoice.postedDateTime}', '${invoice.clientId}',`
    );
    sql.push(
      `'${invoice.clientName}', '${invoice.posTerminal}', '${invoice.gps}', ${invoice.totalAmount},`
    );
    sql.push(
      `'${invoice.erpInvoiceId}', ${invoice.isPosted}, ${invoice.status}, ${invoice.isCreditNote},`
    );
    sql.push(
      `'${invoice.voidReason}', '${invoice.voidNotes}', ${
        invoice.voidInvoiceId === 0 ? null : invoice.voidInvoiceId
      },`
    );
    sql.push(
      `${invoice.printRequests}, ${invoice.printedCount}, '${invoice.authId}', '${invoice.satSerie}',`
    );
    sql.push(
      `${invoice.change}, '${invoice.img1}', '${invoice.img2}', '${invoice.img3}', NULL,`
    );
    sql.push(
      `${invoice.isPaidConsignment}, '${invoice.initialTaskImage}', ${invoice.idBo},`
    );
    sql.push(
      `${invoice.inPlanRoute}, ${invoice.isPostedValidated}, ${invoice.detailQty},`
    );
    sql.push(
      `${invoice.handleTax}, ${invoice.taxPercent}, '${invoice.telephoneNumber}',`
    );
    sql.push(
      `'${invoice.isFromDeliveryNote}', ${invoice.discount}, '${invoice.comment}',`
    );
    sql.push(
      `'${invoice.dueDate}', ${invoice.creditAmount}, ${invoice.cashAmount}, ${invoice.paidToDate},`
    );
    sql.push(
      `${invoice.taskId}, ${
        !invoice.goalHeaderId || invoice.goalHeaderId === 0
          ? "NULL"
          : invoice.goalHeaderId
      }, ${
    ...
    sql.push(
      `${felData.DocumentSeries ? `'${felData.DocumentSeries}'` : "NULL"}, ${
        felData.DocumentNumber
      }, ${felData.DocumentUrl ? `'${felData.DocumentUrl}'` : "NULL"},`
    );
    sql.push(
      `${felData.Shipment}, ${felData.ValidationResult ? 1 : 0}, ${
        !felData.ShipmentDatetime ? "NULL" : `'${felData.ShipmentDatetime}'`
      },`
    );
    sql.push(
      `'${felData.ShipmentResponse}', ${felData.IsContingencyDocument ? 1 : 0},`
    );
    sql.push(
      `${
