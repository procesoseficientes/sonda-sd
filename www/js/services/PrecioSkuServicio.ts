...
class PrecioSkuServicio implements IPrecioSkuServicio {
    obtenerPreciosDePaquetes(cliente: Cliente, sku: Sku, paquetes: Paquete[],decimales:ManejoDeDecimales, callback: (paquetes: Paquete[]) => void, callbackError: (resultado: Operacion) => void) {
        for (var i = 0; i < paquetes.length; i++) {
            this.obtenerPrecioDePaquete(cliente, sku, paquetes[i], i,decimales, (paquete: Paquete, index: number) => {
                //paquetes[index].price = trunc_number(paquete.price,decimales.defaultCalculationsDecimals);
    ...
    obtenerPrecioDePaquete(cliente: Cliente, sku: Sku, paquete: Paquete, index: number,decimales:ManejoDeDecimales, callback: (paquete: Paquete, index: number) => void, callbackError: (resultado: Operacion) => void) {        
        SONDA_DB_Session.transaction(
            (tx) => {
                let qty = (paquete.qty === 0 ? 1 : paquete.qty);
