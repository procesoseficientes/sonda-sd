class SocketsGlobalesServicio {
  ...
  delegarSocketsGlobales(socketIo: SocketIOClient.Socket) {
    ...
    socketIo.on("ValidateRoute_error", data => {
      try {
      ...
      try {
        ...
        switch (data.option) {
            ...
            if (data.row.DOC_TYPE === "ONLINE_INVOICE") {
              ...
              menuControlador.cargarInformacionFel(
                ...
                (display, implementaFel, secuenciaDocumento) => {
                  menuControlador.seValidoCorrectamente(
                    display,
                    secuenciaDocumento
                  );
                },
                error => {
                  notify(
                    `No se pudo validar si usará FEL debido a: ${error.mensaje}`
                  );
                }
              );
            }
          ...
          case "add_statistic_costumer":
            this.estadisticaDeVentaServicio.agregarEstadisticaDeVentaPorCliente(
              data.row
            );
            break;
      ...
      try {
        ...
        if (UsuarioFacturaEnRuta(data.row)) {
          ...
          if (data.doctype == "FACTURA") {
            ...
            localStorage.setItem(
              "FEL_DOCUMENT_TYPE",
              data.row.FEL_DOCUMENT_TYPE
            );

            localStorage.setItem(
              "FEL_STABLISHMENT_CODE",
              data.row.FEL_STABLISHMENT_CODE
            );
          }
        } else {
          $("#lblCompanyName").text(data.row.NAME_ENTERPRISE);
