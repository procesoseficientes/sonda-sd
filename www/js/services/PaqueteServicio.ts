...
class PaqueteServicio implements IPaqueteServicio {
    ...
    obtenerDenominacionesPorSku(sku: Sku, configuracionDecimales: ManejoDeDecimales, cliente: Cliente, puedeEspecificarUm: boolean, callback: (paquetes: Paquete[]) => void, callbackError: (resultado: Operacion) => void) {
        this.obtenerUnidadMinimaDeVenta(sku, cliente, configuracionDecimales, (pacUnidadMinim) => {
            ...
            SONDA_DB_Session.transaction(
                (tx) => {
                    var sql = "SELECT" +
                        ...
                        " LEFT JOIN ITEM_HISTORY IH ON (C1.CODE_SKU= IH.CODE_SKU AND IH.CODE_CUSTOMER='" + cliente.clientId + "' AND IH.DOC_TYPE='TAKE_INVENTORY' AND IH.CODE_PACK_UNIT=C2.CODE_PACK_UNIT_FROM)" +
                        " WHERE C1.CODE_SKU = '" + sku.sku + "' " +
                        " AND C1.[ORDER] = 1" +
    ...
    obtenerUnidadMinimaDeVenta(sku: Sku, cliente: Cliente, configuracionDecimales: ManejoDeDecimales, callback: (paqueteConversion: Paquete) => void, callbackError: (op: Operacion) => void) {
        ...
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT PC.*  FROM PACK_CONVERSION PC INNER JOIN PRICE_LIST_BY_SKU_PACK_SCALE PL ON (PC.CODE_PACK_UNIT_FROM = PL.CODE_PACK_UNIT AND  PC.CODE_SKU= PL.CODE_SKU)"
                    + " WHERE PC.CODE_SKU ='" + sku.sku + "'" + " AND (PL.CODE_PRICE_LIST ='" + cliente.priceListId + "' OR PL.CODE_PRICE_LIST ='" + cliente.priceListId + "') ";
                ...
                tx.executeSql(sql, [],
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            ...
                            umPaquete.descriptionPackUnit = stPaqueteConversion.CODE_PACK_UNIT_FROM;
                            umPaquete.qty = 0;
                            umPaquete.price = 0;
    ...
    obtenerConversionDePaquetes(sku: Sku, configuracionDecimales: ManejoDeDecimales, callback: (paquetesConversion: PaqueteConversion[]) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT "
                    + " C1.CODE_SKU"
                    ...
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            ...
                            for (var i = 0; i < results.rows.length; i++) {
                                ...
                                paquete.codePackUnitFrom = stPaqueteConversion.CODE_PACK_UNIT_FROM;
                                paquete.descriptionPackUnitFrom = stPaqueteConversion.DESCRIPTION_PACK_UNIT_FROM;
                                paquete.codePackUnitTo = stPaqueteConversion.CODE_PACK_UNIT_TO;
                                paquete.descriptionPackUnitTo = stPaqueteConversion.DESCRIPTION_PACK_UNIT_TO;
                            ...
                            operacion.resultado = ResultadoOperacionTipo.Error;
                            operacion.codigo = 0;
                            operacion.mensaje = "Este producto no tiene configurado las conversiones.";
                            callbackError(operacion);
                        }
    ...
    obtenerConversionDePaquete(sku: Sku, paquetes: Paquete[], index, configuracionDecimales: ManejoDeDecimales, callback: (paqueteConversion: PaqueteConversion, paquetes: Paquete[], index: number) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT C1.CODE_SKU" +
                    ...
                    " WHERE C1.CODE_SKU = '" + sku.sku + "' AND C1.CODE_PACK_UNIT_FROM ='" + paquetes[index].codePackUnit + "' LIMIT 1";
                ...
                tx.executeSql(sql, [],
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length > 0) {
                            ...
                            paqueteConversion.conversionFactor = stPaqueteConversion.CONVERSION_FACTOR;
                            paqueteConversion.codePackUnitFrom = stPaqueteConversion.CODE_PACK_UNIT_FROM;
                            paqueteConversion.codePackUnitTo = stPaqueteConversion.CODE_PACK_UNIT_TO;
    ...
    obtenerDenominacionPorSku(sku: Sku, indiceDeLista: number, callback: (listaPaquete: Paquete[], indiceDeLista: number) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT" +
                    " U2.PACK_UNIT" +
                ...
                tx.executeSql(sql, [],
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            ...
                            operacion.resultado = ResultadoOperacionTipo.Error;
                            operacion.codigo = 0;
                            operacion.mensaje = "Este producto no tiene denominacion configurado.";
                            callbackError(operacion);
                        }
                    }
                );
    ...
    obtenerTodasLasUnidadesDeMedida(callback: (paquetes: Paquete[])=>void, callbackError:(resultado: Operacion)=>void) {
        SONDA_DB_Session.transaction(
            tx => {
                ...
                tx.executeSql(sql, [],
                    ...
                    (tx2, err) => {
                        if (err.code !== 0) {
                            callbackError(<Operacion>{ codigo: err.code, mensaje: `Error al obtener unidades de medida de skus: ${err.message}` });
                        }
                    }
                );
            },
