class PagoDeFacturaVencidaServicio implements IPagoDeFacturaVencidaServicio {
  impresionServicio: ImpresionServicio = new ImpresionServicio();

  guardarDocumentoDePago(
    ...
    try {
      SONDA_DB_Session.transaction(
        (trans: SqlTransaction) => {
          trans.executeSql(
            this.obtenerFormatoSqlDeInsercionDeEncabezadoDeDocumentoDePago(
              documentoDePago
            )
          );

          documentoDePago.overdueInvoicePaymentDetail.forEach(
            (detalleDeDocumentoDePago: PagoDeFacturaVencidaDetalle) => {
              trans.executeSql(
                this.obtenerFormatoSqlDeInsercionDeDetalleDeDocumentoDePago(
                  detalleDeDocumentoDePago
                )
              );
            }
          );
          ...
          documentoDePago.overdueInvoicePaymentDetail.forEach(
            (detalleDeDocumentoDePago: PagoDeFacturaVencidaDetalle) => {
              trans.executeSql(
                this.obtenerFormatoSqlDeActualizacionDeMontoPendienteDePagoEnFacturaOriginal(
                  detalleDeDocumentoDePago
                )
              );
            }
          ...
          documentoDePago.overdueInvoicePaymentTypeDetail.forEach(
            (tipoDePago: TipoDePagoEnFacturaVencida) => {
              trans.executeSql(
                this.obtenerFormatoSqlDeInsercionDeTipoDePago(tipoDePago)
              );
            }
          );

          trans.executeSql(
            this.obtenerFormatoDeActualizacionDeBalanceDeCliente(
              documentoDePago
            )
          );

          trans.executeSql(
            this.obtenerFormatoDeActualizacionDeSecuenciaDeDocumentos(
              documentoDePago
            )
          );
        },
  ...
  obtenerFormatoSqlDeInsercionDeEncabezadoDeDocumentoDePago(
    documentoDePago: PagoDeFacturaVencidaEncabezado
  ): string {
    let sql: Array<string> = [];

    sql.push(
      "INSERT INTO OVERDUE_INVOICE_PAYMENT_HEADER(CODE_CUSTOMER, DOC_SERIE, DOC_NUM"
    );
    sql.push(
      " ,CREATED_DATE, CODE_ROUTE, LOGIN_ID, PAYMENT_AMOUNT, IS_POSTED, COMMENT, PAYMENT_APPLIED_TO)"
    );
    sql.push(" VALUES(");
  ...
  obtenerFormatoSqlDeInsercionDeDetalleDeDocumentoDePago(
    detalleDeDocumentoDePago: PagoDeFacturaVencidaDetalle
  ): string {
    let sql: Array<string> = [];

    sql.push(
      "INSERT INTO OVERDUE_INVOICE_PAYMENT_DETAIL(INVOICE_ID, DOC_ENTRY, DOC_SERIE, DOC_NUM, PAYED_AMOUNT, AMOUNT_TO_DATE, PENDING_AMOUNT)"
    );
    sql.push(" VALUES(");
  ...
  obtenerDocumentosDePagoNoPosteadosEnElServidor(
    callback: (
      documentosDePagoNoPosteadosEnElServidor: PagoDeFacturaVencidaEncabezado[]
    ) => void
  ): void {
    this.obtenerEncabezadoDeDocumentosNoPosteadosEnElServidor(callback);
  }

  obtenerEncabezadoDeDocumentosNoPosteadosEnElServidor(
    callback: (
      documentosDePagoNoPosteadosEnElServidor: PagoDeFacturaVencidaEncabezado[]
    ) => void
  ): void {
    ...
    sql.push(
      `SELECT CODE_CUSTOMER, DOC_SERIE, DOC_NUM, CREATED_DATE, CODE_ROUTE`
    );
    sql.push(
      `, LOGIN_ID, PAYMENT_AMOUNT, COMMENT FROM OVERDUE_INVOICE_PAYMENT_HEADER`
    );
      ...
      (trans: SqlTransaction) => {
        trans.executeSql(
          ...
          (transResult: SqlTransaction, results: SqlResultSet) => {
            ...
            this.obtenerDetalleDeDocumentosDePagoNoPosteadosEnElServidor(
              documentosDePagoNoPosteados,
              0,
              (
                documentosDePago: PagoDeFacturaVencidaEncabezado[],
                transaccionActual: SqlTransaction
              ) => {
                this.obtenerDetalleDeTiposDePagoDeDocumentosDePagoNoPosteadosEnElServidor(
                  documentosDePago,
                  0,
                  callback,
                  transaccionActual
          ...
          (transResult: SqlTransaction, error: SqlError) => {
            console.log(
              `Error al obtener los documentos de pago de facturas vencidas no posteados debido a: '${error.message}'`
            );
          }
        );
      },
  ...
  obtenerDetalleDeDocumentosDePagoNoPosteadosEnElServidor(
    ...
    callback: (
      documentosDePagoNoPosteadosEnElServidor: PagoDeFacturaVencidaEncabezado[],
      transaccionSql: SqlTransaction
    ) => void,
    transaccionActual: SqlTransaction
  ): void {
    if (
      documentosDePagoNoPosteadosEnElServidor.length >
      posicionActualDeDocumentoProcesado
    ) {
      let sql: Array<string> = [];

      sql.push(
        "SELECT INVOICE_ID, DOC_ENTRY, DOC_SERIE, DOC_NUM, PAYED_AMOUNT, AMOUNT_TO_DATE, PENDING_AMOUNT FROM OVERDUE_INVOICE_PAYMENT_DETAIL"
      );
      sql.push(
        ` WHERE DOC_SERIE = '${documentosDePagoNoPosteadosEnElServidor[posicionActualDeDocumentoProcesado].docSerie}'`
      );
      sql.push(
        ` AND DOC_NUM = ${documentosDePagoNoPosteadosEnElServidor[posicionActualDeDocumentoProcesado].docNum}`
      );
      ...
      transaccionActual.executeSql(
        ...
        (transResult: SqlTransaction, results: SqlResultSet) => {
          for (let i: number = 0; i < results.rows.length; i++) {
            ...
            documentosDePagoNoPosteadosEnElServidor[
              posicionActualDeDocumentoProcesado
            ].overdueInvoicePaymentDetail.push(detalle);
          }
          this.obtenerDetalleDeDocumentosDePagoNoPosteadosEnElServidor(
            documentosDePagoNoPosteadosEnElServidor,
            posicionActualDeDocumentoProcesado + 1,
            callback,
            transResult
          );
        },
        (transResult: SqlTransaction, error: SqlError) => {
          console.log(
            `Error al obtener el detalle de los documentos de pago de facturas vencidas no posteados debido a: '${error.message}'`
          );
        }
      );
    } else {
  ...
  marcarDocumentosDePagoComoPosteadosEnElServidor(
    documentosDePagoPosteadosEnElServidor: any
  ): void {
    try {
      if (documentosDePagoPosteadosEnElServidor.length > 0) {
        SONDA_DB_Session.transaction(
          (trans: SqlTransaction) => {
            documentosDePagoPosteadosEnElServidor.forEach(
              (documentoPosteado: any) => {
                if (
                  documentoPosteado.RESULT ===
                  ResultadoDePosteoEnServidor.Exitoso
                ) {
                  trans.executeSql(
                    this.obtenerFormatoDeActualizacionDePosteoDeEncabezadoDeDocumentoDePago(
                      documentoPosteado
                    )
                  );

                  trans.executeSql(
                    this.obtenerFormatoDeActualizacionDePosteoDeDetalleDeDocumentoDePago(
                      documentoPosteado
                    )
                  );

                  trans.executeSql(
                    this.obtenerFormatoDeActualizacionDePosteoDeDetalleDeTipoDePagoDeDocumentoDePago(
                      documentoPosteado
                    )
                  );
                }
          ...
          (error: SqlError) => {
            console.log(
              `Error al marcar el pago de factura vencida como posteado en el servidor debido a: '${error.message}'`
            );
          }
        );
      }
      ...
      console.log(
        `Error al actualizar los documentos de pago de facturas vencidas posteados en el servidor debido a: ${e.message}`
      );
    }
  }

  obtenerFormatoDeActualizacionDePosteoDeEncabezadoDeDocumentoDePago(
    documentoPosteado: any
  ): string {
    let sql: Array<string> = [];
    ...
    sql.push(
      ` SET IS_POSTED = ${SiNo.Si}, ID = ${documentoPosteado.PAYMENT_ID_BO}`
    );
    sql.push(
      ` WHERE DOC_SERIE = '${documentoPosteado.DOC_SERIE}' AND DOC_NUM = ${documentoPosteado.DOC_NUM}`
    );

    return sql.join("");
  }

  obtenerFormatoDeActualizacionDePosteoDeDetalleDeDocumentoDePago(
    documentoPosteado: any
  ): string {
    let sql: Array<string> = [];

    sql.push(
      `UPDATE OVERDUE_INVOICE_PAYMENT_DETAIL SET PAYMENT_HEADER_ID = ${documentoPosteado.PAYMENT_ID_BO} `
    );
    sql.push(
      ` WHERE DOC_SERIE = '${documentoPosteado.DOC_SERIE}' AND DOC_NUM = ${documentoPosteado.DOC_NUM}`
    );
  ...
  imprimirPago(
    ...
    this.impresionServicio.obtenerFormatoDeImpresionDePago(
      pago,
      formatoDeImpresion => {
  ...
  obtenerFormatoDeActualizacionDeSecuenciaDeDocumentos(
    documentoDePago: PagoDeFacturaVencidaEncabezado
  ): string {
    let sql: Array<string> = [];

    sql.push(
      `UPDATE DOCUMENT_SEQUENCE SET CURRENT_DOC = ${documentoDePago.docNum}`
    );
    sql.push(
      ` WHERE SERIE='${documentoDePago.docSerie}' AND DOC_TYPE = '${TIpoDeDocumento.PagoDeFacturaVencida}'`
    );

    return sql.join("");
  }

  obtenerParametroDePorcentajeDePagoMinimoDeFacturasVencidas(
    callback: (
      aplicaParametroDePorcentajeMinimoDePago: boolean,
      valorDePorcentajeMinimoDePago: number
    ) => void,
    errorCallback: (resultado: Operacion) => void
  ): void {
    try {
      let parametroPorcentajeMinimoDePago: any = localStorage.getItem(
        "MINIMUM_PERCENTAGE_OF_PAYMENT"
      );
  ...
  obtenerFormatoSqlDeActualizacionDeMontoPendienteDePagoEnFacturaOriginal(
    detalleDeDocumentoDePago: PagoDeFacturaVencidaDetalle
  ): string {
    let sql: Array<string> = [];

    sql.push(
      `UPDATE OVERDUE_INVOICE_BY_CUSTOMER SET PENDING_TO_PAID = PENDING_TO_PAID - ${detalleDeDocumentoDePago.payedAmount}`
    );
    sql.push(
      ` WHERE INVOICE_ID = '${detalleDeDocumentoDePago.invoiceId}' AND DOC_ENTRY = '${detalleDeDocumentoDePago.docEntry}'`
    );

    return sql.join("");
  }

  obtenerSecuenciaDeDocumentoDePago(
    callback: (secuenciaDeDocumento: any) => void,
    errorCallback: (resultado: Operacion) => void
  ): void {
    let sql: Array<string> = [];
    sql.push(
      `SELECT SERIE, (CURRENT_DOC + 1) AS CURRENT_DOC, BRANCH_NAME, BRANCH_ADDRESS `
    );
    sql.push(
      ` FROM DOCUMENT_SEQUENCE WHERE DOC_TYPE = '${TIpoDeDocumento.PagoDeFacturaVencida}'`
    );
  ...
  obtenerFormatoSqlDeInsercionDeTipoDePago(
    tipoDePago: TipoDePagoEnFacturaVencida
  ): string {
    let sql: Array<string> = [];
  ...
  obtenerDetalleDeTiposDePagoDeDocumentosDePagoNoPosteadosEnElServidor(
    documentosDePagoNoPosteadosEnElServidor: PagoDeFacturaVencidaEncabezado[],
    posicionActualDeDocumentoProcesado: number,
    callback: (
      documentosDePagoNoPosteadosEnElServidor: PagoDeFacturaVencidaEncabezado[]
    ...
    if (
      ...
      transaccionActual.executeSql(
        ...
        (transResult: SqlTransaction, error: SqlError) => {
          console.log(
            `Error al obtener el detalle de los tipos de pago, para los documentos de pago de facturas vencidas no posteados debido a: '${error.message}'`
          );
        }
      );
    } else {
  ...
  obtenerFormatoDeActualizacionDePosteoDeDetalleDeTipoDePagoDeDocumentoDePago(
    documentoPosteado: any
  ): string {
    let sql: Array<string> = [];

    sql.push(
      `UPDATE OVERDUE_INVOICE_PAYMENT_TYPE_DETAIL SET PAYMENT_HEADER_ID = ${documentoPosteado.PAYMENT_ID_BO} `
    );
    sql.push(
      ` WHERE DOC_SERIE = '${documentoPosteado.DOC_SERIE}' AND DOC_NUM = ${documentoPosteado.DOC_NUM}`
    );
  ...
  obtenerEncabezadoDeDocumentosDePagoParaReporte(
    callback: (documentosDePago: PagoDeFacturaVencidaEncabezado[]) => void,
    errorCallbak: (resultado: Operacion) => void
  ): void {
    ...
    SONDA_DB_Session.transaction(
      (trans: SqlTransaction) => {
        ...
        sql.push(
          ` , (SELECT C.CLIENT_NAME FROM CLIENTS AS C WHERE C.CLIENT_ID = OIPH.CODE_CUSTOMER LIMIT 1) AS NAME_CUSTOMER`
        );
        sql.push(
          ` FROM OVERDUE_INVOICE_PAYMENT_HEADER AS OIPH ORDER BY OIPH.DOC_NUM ASC`
        );
        ...
        trans.executeSql(
          ...
          (transResult: SqlTransaction, results: SqlResultSet) => {
            for (let i: number = 0; i < results.rows.length; i++) {
              ...
              let pago: PagoDeFacturaVencidaEncabezado = new PagoDeFacturaVencidaEncabezado();
            ...
            this.obtenerDetalleDeDocumentosDePagoParaReporte(
              documentosDePago,
              0,
              (
                documentosDePagoConDetalle: PagoDeFacturaVencidaEncabezado[],
                transaccion: SqlTransaction
              ) => {
                this.obtenerDetalleDeTiposDePagoDeDocumentosDePagoParaReporte(
                  documentosDePagoConDetalle,
                  0,
                  callback,
                  errorCallbak,
                  transaccion
  ...
  obtenerDetalleDeDocumentosDePagoParaReporte(
    documentosDePago: PagoDeFacturaVencidaEncabezado[],
    posicionActualDeDocumentoProcesado: number,
    callback: (
      documentosDePago: PagoDeFacturaVencidaEncabezado[],
    ...
    if (documentosDePago.length > posicionActualDeDocumentoProcesado) {
      ...
      sql.push(
        ` INNER JOIN OVERDUE_INVOICE_BY_CUSTOMER OIC ON(OIC.INVOICE_ID = OIPD.INVOICE_ID AND OIC.DOC_ENTRY = OIPD.DOC_ENTRY)`
      );
      sql.push(
        ` WHERE OIPD.DOC_SERIE = '${documentosDePago[posicionActualDeDocumentoProcesado].docSerie}'`
      );
      sql.push(
        ` AND OIPD.DOC_NUM = ${documentosDePago[posicionActualDeDocumentoProcesado].docNum}`
      );
      ...
      transaccionActual.executeSql(
        ...
        (transResult: SqlTransaction, error: SqlError) => {
          console.log(
            `Error al obtener el detalle de los documentos de pago debido a: '${error.message}'`
          );
          errorCallback({
            codigo: error.code,
            resultado: ResultadoOperacionTipo.Error,
  ...
  obtenerDetalleDeTiposDePagoDeDocumentosDePagoParaReporte(
    documentosDePago: PagoDeFacturaVencidaEncabezado[],
    posicionActualDeDocumentoProcesado: number,
    callback: (documentosDePago: PagoDeFacturaVencidaEncabezado[]) => void,
    errorCallback: (resultado: Operacion) => void,
    ...
    if (documentosDePago.length > posicionActualDeDocumentoProcesado) {
      ...
      sql.push(` FROM OVERDUE_INVOICE_PAYMENT_TYPE_DETAIL`);
      sql.push(
        ` WHERE DOC_SERIE = '${documentosDePago[posicionActualDeDocumentoProcesado].docSerie}'`
      );
      sql.push(
        ` AND DOC_NUM = ${documentosDePago[posicionActualDeDocumentoProcesado].docNum}`
      );
  ...
  obtenerInformacionDeSecuenciaDeDocumentoDePago(
    callback: (secuenciaDeDocumento: any) => void,
    errorCallback: (resultado: Operacion) => void
  ): void {
    let sql: Array<string> = [];
  ...
  obtenerFormatoDeActualizacionDeBalanceDeCliente(
    pago: PagoDeFacturaVencidaEncabezado
  ): string {
    var formato: string[] = [];

    formato.push(
      `UPDATE CLIENTS SET OUTSTANDING_BALANCE = OUTSTANDING_BALANCE + ${pago.paymentAmount}, PREVIUS_BALANCE = PREVIUS_BALANCE - ${pago.paymentAmount}`
    );
