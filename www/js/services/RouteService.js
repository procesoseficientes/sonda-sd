...
function AddToPosSku(data) {
    ...
    pSql = "INSERT INTO SKUS(SKU, SKU_NAME, SKU_PRICE, SKU_LINK, REQUERIES_SERIE, IS_KIT, ON_HAND, ROUTE_ID, IS_PARENT, PARENT_SKU, EXPOSURE, PRIORITY, QTY_RELATED, LOADED_LAST_UPDATED, CODE_FAMILY_SKU, SALES_PACK_UNIT)";
    pSql += "VALUES('" + data.row.SKU + "','" + data.row.SKU_NAME + "'," + format_number(data.row.SKU_PRICE, 2) + ",'...',";
    pSql += data.row.REQUERIES_SERIE + "," + data.row.IS_KIT + "," + data.row.ON_HAND + ",'" + data.row.ROUTE_ID + "',";
    pSql += data.row.IS_PARENT + ",'" + data.row.PARENT_SKU + "'," + data.row.EXPOSURE + "," + data.row.PRIORITY + ",";
    pSql += data.row.QTY_RELATED + ",'" + getDateTime() + "', '" + data.row.CODE_FAMILY_SKU + "', '" + data.row.SALESSALES_PACK_UNIT + "')";
    //console.log(pSql);
    window.gInsertsInitialRoute.push(pSql);
    //        tx.executeSql(pSql);
...
function PosSkusCompleted(data) {
    ...
    if (gDefaultWhs === "") {
        my_dialog("", "", "close");
        notify("ERROR, No hay ruta actual definida, contacte a su Administrador de Sonda.");
        return;
    }
}

function RequestedSerie(data) {
    //ToastThis("obteniendo series...");
    ...
    //    function (tx, err) {
    //        my_dialog("", "", "close");
    //        notify("requested_series: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function AddToSeries(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = "DELETE FROM SKU_SERIES WHERE SKU = '" + data.row.SKU + "' AND SERIE = '" + data.row.SKU_SERIE + "'";
    window.gInsertsInitialRoute.push(pSql);
    //tx.executeSql(pSql);
    pSql = "INSERT INTO SKU_SERIES(SKU, IMEI, SERIE, PHONE, ICC, STATUS, LOADED_LAST_UPDATED)";
    pSql += "VALUES('" + data.row.SKU + "','" + data.row.SKU_ICC + "','" + data.row.SKU_SERIE + "','" + data.row.SKU_PHONE + "',";
...
function GetInitialRouteCompleted(data) {
    ...
    InsertarRegistrosBDCompleta(function () {
...
function InsertarRegistrosBDCompleta(callBack, errorCallBack) {
    try {
        SONDA_DB_Session.transaction(function (trans) {
            for (var i = 0; i < window.gInsertsInitialRoute.length; i++) {
                var sql = window.gInsertsInitialRoute[i];
                console.log(sql);
...
function RequestedTags(data) {
    //ToastThis("obteniendo etiquetas...");
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        notify("requested_tags: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function AddToTags(data) {
    ...
    pSql += "VALUES('" + data.row.TAG_COLOR + "','" + data.row.TAG_VALUE_TEXT + "','" + data.row.TAG_PRIORITY + "','" + data.row.TAG_COMMENTS + "')";
    window.gInsertsInitialRoute.push(pSql);
    //        tx.executeSql(pSql);
...
function RequestedGetCustomer(data) {
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        notify("requested_get_customer: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function AddToCustomer(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = ""; //"DELETE FROM CLIENTS WHERE CLIENT_ID = '" + data.row.CODE_CUSTOMER + "'";
    //tx.executeSql(pSql);
...
function AddToCustomerFrequency(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = ""; //"DELETE FROM CLIENTS_FREQUENCY WHERE CODE_CUSTOMER = '" + data.row.CODE_CUSTOMER + "'";
    //tx.executeSql(pSql);
...
function RequestedGetTagsXCustomer(data) {
    //ToastThis("obteniendo etiquepas por clientes...");
...
function AddToTagsXCustomer(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = ""; //"DELETE FROM TAGS_X_CUSTOMER WHERE CUSTOMER = '" + data.row.CUSTOMER + "' AND TAG_COLOR = '" + data.row.TAG_COLOR + "'";
    //tx.executeSql(pSql);
...
function RequestedGetRules(data) {
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        notify("requested_get_event: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function AddToRule(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = ""; //"DELETE FROM RULE WHERE EVENT_ID = " + data.row.EVENT_ID;
    //tx.executeSql(pSql);
    ...
    try {
        ...
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                var pSql = " DELETE FROM REASONS WHERE REASON_VALUE = '" + data.NAME_CLASSIFICATION + "'";
                tx.executeSql(pSql);
...
function RequestedGetSkuPreSale(data) {
    //ToastThis("obteniendo sku para orden de venta...");
...
function NoGetSkuPreSaleFound(data) {
    ToastThis("No se econtraron sku para la orden de venta, para la ruta: " + data.routeid);
}
...
function AddToSkuPreSale(data) {
    ...
    var pSql = "";//"DELETE FROM SKU_PRESALE WHERE WAREHOUSE = '" + data.row.WAREHOUSE + "' AND SKU = '" + data.row.SKU + "'";
    //tx.executeSql(pSql);
    ...
    if (data.row.HANDLE_DIMENSION === null ||
...
function GetDocumentSequenceStart(data) {
    //ToastThis("Obteniendo secuencias de documentos...");
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        notify("Error en Secuancia de Documentos : " + err.message);
    //    },
    //    function () { }
    //);
}
...
function GetDocumentSequenceNoFound(data) {
    ToastThis("No se encontraron secuencias de documentos, para la ruta: " + data.routeid);
}

function AddToDocumentSequence(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = "";//"DELETE FROM DOCUMENT_SEQUENCE WHERE DOC_TYPE = '" + data.row.DOC_TYPE + "' AND SERIE = '" + data.row.SERIE + "'";
    //tx.executeSql(pSql);
...
function AddToPackUnit(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = ""; //"DELETE FROM PACK_UNIT WHERE CODE_PACK_UNIT = '" + data.row.CODE_PACK_UNIT + "'";
    //tx.executeSql(pSql);
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        alert("es en paquetes: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function GetPackConversionStart(data) {
    //ToastThis("Obteniendo conversion de paquetes...");
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        notify("Error en Conversion de Paquetes: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function AddToPackConversion(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = ""; //"DELETE FROM PACK_CONVERSION WHERE PACK_CONVERSION = '" + data.row.PACK_CONVERSION + "'";
    //tx.executeSql(pSql);
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        //alert("Es en conversiones: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function MostarInformacionDeUsuario(data) {
    ...
    if (data.SELLER_OWNER !== '') localStorage.setItem("SELLER_OWNER", data.SELLER_OWNER);
    if (data.SELLER_OWNER_ID !== 0) localStorage.setItem("SELLER_OWNER_ID", data.SELLER_OWNER_ID);
    else {
        var data1 = {
        ...
        console.log("SELLER_OWNER: " + data.SELLER_OWNER);
        console.log(data1);
...
function MostarSecuenciaDeDocumentos(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        vLI += '<span class="ui-li-count">' + data[i].POST_DATETIME + '</span> </li>';
...
function MostarResumenDeTareas(data) {
    ...
    console.log("MostarResumenDeTareas");
    console.dir(data);
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        vLI += '<span class="small-roboto ui-li-count">' + data[i].TOTAL_TASK_TYPE + '</span>';
        vLI += '</li>';
...
function RequestedGetFamilySku(data) {
    //ToastThis("Obteniendo familias de sku...");
...
function AddToFamilySku(data) {
    //SONDA_DB_Session.transaction(
    //            function (tx) {
    var pSql = ""; //"DELETE FROM FAMILY_SKU WHERE FAMILY_SKU = " + data.row.FAMILY_SKU;
    //tx.executeSql(pSql);
...
function AddInvoice(data) {
    SONDA_DB_Session.transaction(
                function (tx) {
                    ...
                    var pSql = "DELETE FROM INVOICE_HEADER WHERE INVOICE_NUM = " + data.invoiceId;
...
function GetInvoiceCompleted(data) {
    //ToastThis("Facturas cargadas exitosamente");
...
function PriceListByCustomerReceived() {
    ...
    //ToastThis("Recibiendo Listas de Precio por Cliente");
}

function PriceListByCustomerNotFound(data) {
    ToastThis("No se encontraron Listas de Precio por Cliente para la Ruta: " + data.routeid);
}
...
function AddPriceListByCustomer(data) {
    ...
    //pSql = "DELETE FROM PRICE_LIST_BY_CUSTOMER WHERE CODE_PRICE_LIST = '" + data.row.CODE_PRICE_LIST + "'";
...
function PriceListBySKUReceived() {
    ...
    window.gInsertsInitialRoute.push(pSql);
    //ToastThis("Recibiendo Listas de Precio por SKU");
}

function PriceListBySKUNotFound(data) {
    ToastThis("No se encontraron Listas de Precio por SKU para la Ruta: " + data.routeid);
}
...
function AddPriceListBySKU(data) {
    ...
    //        pSql = "DELETE FROM PRICE_LIST_BY_SKU WHERE CODE_PRICE_LIST = '" + data.row.CODE_PRICE_LIST + "'";
...
function GetItemHistoryReceived() {
    var pSql = "DELETE FROM ITEM_HISTORY";
    window.gInsertsInitialRoute.push(pSql);
    //ToastThis("Recibiendo Historial de Ventas.");
}
...
function AddItemHistory(data) {
    ...
    //        pSql = "DELETE FROM ITEM_HISTORY";
    //        pSql += " WHERE DOC_TYPE = '" + data.row.DOC_TYPE + "'";
    //        pSql += " AND CODE_CUSTOMER = '" + data.row.CODE_CUSTOMER + "'";
    //        pSql += " AND CODE_SKU = '" + data.row.CODE_SKU + "'";
    //        pSql += " AND CODE_PACK_UNIT = '" + data.row.CODE_PACK_UNIT + "'";
    //        tx.executeSql(pSql);
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        alert("No se pudo insertar el Historial de Ventas debido al error: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function AddSalesOrderDraft(data) {
    ...
    pSql = "DELETE FROM SALES_ORDER_HEADER WHERE SALES_ORDER_ID = " + data.data.SALES_ORDER_ID_HH;
    ...
    pSql = "DELETE FROM SALES_ORDER_DETAIL WHERE SALES_ORDER_ID = " + data.data.SALES_ORDER_ID_HH;
    window.gInsertsInitialRoute.push(pSql);
    //tx.executeSql(pSql);
    //pSql = null;
    ...
    //pSql += " , ATTEMPTED_WITH_ERROR";
    //pSql += " , IS_POSTED_ERP";
    //pSql += " , POSTED_ERP";
    //pSql += " , POSTED_RESPONSE";
    pSql += " , IS_PARENT";
    pSql += " , REFERENCE_ID";
    //pSql += " , WAREHOUSE";
    pSql += " , TIMES_PRINTED";
    ...
    //pSql += " , '" + data.data.ATTEMPTED_WITH_ERROR + "'";
    //pSql += " , '" + data.data.IS_POSTED_ERP + "'";
    //pSql += " , '" + data.data.POSTED_ERP + "'";
    //pSql += " , '" + data.data.POSTED_RESPONSE + "'";
    pSql += " , 1";
    pSql += " , '" + data.data.REFERENCE_ID + "'";
    //pSql += " , '" + data.data.WAREHOUSE + "'";
...
function AddInvoiceDraft(data) {
    ...
    var pSql = "DELETE FROM INVOICE_HEADER WHERE INVOICE_NUM = " + data.INVOICE_ID;
    ...
    for (var i = 0; i < data.details.length; i++) {
        ...
        pSql += " , " + detalle.REQUERIES_SERIE;
        pSql += " , '" + detalle.COMBO_REFERENCE + "'";
        //pSql += " , '" + detalle.INVOICE_RESOLUTION + "'";
        pSql += " , " + detalle.PARENT_SEQ;
        //pSql += " , " + detalle.IS_ACTIVE_ROUTE;
        pSql += " )";
        //tx.executeSql(pSql);
...
function AddDefaultPackSku(data) {
    ...
    //    function (err) {
    //        my_dialog("", "", "close");
    //        alert("No se pudo insertar la Unidad de Paquete Default de SKU debido al error: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function PriceListBySkuPackScaleReceived() {
    //ToastThis("Recibiendo listas de precios por SKU y unidad de medida");

    //SONDA_DB_Session.transaction(
    //    function (tx) {
    var pSql = "DELETE FROM PRICE_LIST_BY_SKU_PACK_SCALE";
    window.gInsertsInitialRoute.push(pSql);
    //        tx.executeSql(pSql);
    //    },
    //    function (err) {
    //        my_dialog("", "", "close");
    //        notify("PriceListBySkuPackScaleReceived: " + err.message);
    //    },
    //    function () { }
    //);
}
...
function AddPriceListBySkuPackScale(data) {
    ...
    //        pSql = "DELETE FROM PRICE_LIST_BY_SKU_PACK_SCALE";
    //        pSql += " WHERE CODE_PRICE_LIST = '" + data.row.CODE_PRICE_LIST + "'";
    //        pSql += " AND CODE_SKU = '" + data.row.CODE_SKU + "'";
    //        pSql += " AND CODE_PACK_UNIT = '" + data.row.CODE_PACK_UNIT + "'";
    //        pSql += " AND [PRIORITY] = " + data.row.PRIORITY;
    //        pSql += " AND LOW_LIMIT = " + data.row.LOW_LIMIT;
    //        pSql += " AND HIGH_LIMIT = " + data.row.HIGH_LIMIT;
    //        tx.executeSql(pSql);
...
function BonusListByCustomerNotFound(data) {
    ToastThis("No se encontraron Listas de bonos por Cliente para la Ruta: " + data.routeid);
}
...
function AddBonusListByCustomer(data) {
    ...
    pSql = "DELETE FROM BONUS_LIST_BY_CUSTOMER WHERE BONUS_LIST_ID = '" + data.row.BONUS_LIST_ID + "'";
    pSql += " AND CODE_CUSTOMER = '" + data.row.CODE_CUSTOMER + "'";
    //tx.executeSql(pSql);
    window.gInsertsInitialRoute.push(pSql);
...
function BonusListBySkuNotFound(data) {
    ToastThis("No se encontraron Listas de bonos por sku para la Ruta: " + data.routeid);
}
...
function AddBonusListBySku(data) {
    ...
    pSql = "DELETE FROM BONUS_LIST_BY_SKU WHERE BONUS_LIST_ID = '" + data.row.BONUS_LIST_ID + "'";
    pSql += " AND CODE_SKU = '" + data.row.CODE_SKU + "'";
    //tx.executeSql(pSql);
    window.gInsertsInitialRoute.push(pSql);
...
function DiscountListByCustomerNotFound(data) {
    ToastThis("No se encontraron Listas de descuentos por Cliente para la Ruta: " + data.routeid);
}
...
function AddDiscountListByCustomer(data) {
    ...
    pSql = "DELETE FROM DISCOUNT_LIST_BY_CUSTOMER WHERE DISCOUNT_LIST_ID = '" + data.row.DISCOUNT_LIST_ID + "'";
    pSql += " AND CODE_CUSTOMER = '" + data.row.CODE_CUSTOMER + "'";
    //tx.executeSql(pSql);
    window.gInsertsInitialRoute.push(pSql);
...
function DiscountListByGeneralAmountNotFound(data) {
    ToastThis("No se encontraron Listas de descuentos por Monto General para la Ruta: " + data.routeid);
}

function AddDiscountListByGeneralAmount(data) {
    var listaParaEjecucion = [];
   
    listaParaEjecucion.push("DELETE FROM DISCOUNT_LIST_BY_GENERAL_AMOUNT WHERE DISCOUNT_LIST_ID = '" + data.row.DISCOUNT_LIST_ID + "'");
...
function AddDiscountListBySku(data) {
    ...
    listaParaEjecucion.push("DELETE FROM DISCOUNT_LIST_BY_SKU WHERE DISCOUNT_LIST_ID = '" + data.row.DISCOUNT_LIST_ID + "'");
...
function AgregarBonoPorMontoGeneral(row) {
    var listaDeLi = [];
    listaDeLi.push("INSERT INTO BONUS_LIST_BY_GENERAL_AMOUNT(BONUS_LIST_ID, LOW_LIMIT, HIGH_LIMIT, CODE_SKU_BONUS, CODE_PACK_UNIT_BONUS, BONUS_QTY, PROMO_ID, PROMO_NAME, PROMO_TYPE, FREQUENCY) VALUES");
...
function AgregarHistoricoPorPromo(row) {
    var listaDeLi = [];
    listaDeLi.push("INSERT INTO HISTORY_BY_PROMO([DOC_SERIE], [DOC_NUM], [CODE_ROUTE], [CODE_CUSTOMER], [HISTORY_DATETIME], [PROMO_ID], [PROMO_NAME], [FREQUENCY], [IS_POSTED]) VALUES");
