...
class ClienteServicio implements IClienteServicio {
    ...
    obtenerCliente(cliente: Cliente, configuracionDecimales: ManejoDeDecimales, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void): void {
        SONDA_DB_Session.transaction(
            (tx) => {
                ...
                tx.executeSql(sql, [],
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            ...
                            callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener el cliente: No se puede encontrar el cliente" });
                ...
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener el Cliente: " + err.message });
            }
        );
    }

    obtenerListaDePrecioPorCliente(cliente: Cliente, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT";
                ...
                callbackError(<Operacion>{ codigo: -1, mensaje: "No se pudo obtener la Lista de Precios del Cliente debido al siguiente error: " + err.message });
            }
        );
    }

    obtenerCuentaCorriente(cliente: Cliente, configuracionDecimales: ManejoDeDecimales, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        try {
            let configuracion: ManejoDeDecimales = configuracionDecimales;
            this.obtenerSiTieneFacturasVenciadas(cliente, (cliente: Cliente) => {
                this.obtenerLimiteDeCredito(cliente, configuracion, (cliente: Cliente) => {
                    this.obtenerSiTieneDiasDeCreditoVencidos(cliente, (cliente: Cliente) => {
                        this.obtenerSaldoActual(cliente, configuracion, (cliente: Cliente) => {
                            this.obtenerSaldoDeFacturas(cliente, configuracion, (cliente: Cliente) => {
                                this.obtenerSaldoDeOrdenesDeVenta(cliente, configuracion, (cliente: Cliente) => {
                                    callback(cliente);
                                }, (reultado: Operacion) => {
            ...
            callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener el cuenta corriente: " + err.message });
        }
    }

    obtenerSiTieneFacturasVenciadas(cliente: Cliente, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT ";
                sql += " I.DOC_DUE_DATE";
                ...
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener facturas vencidas: " + err.message });
            }
        );
    }

    obtenerLimiteDeCredito(cliente: Cliente, configuracionDecimales: ManejoDeDecimales, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT ";
                sql += " C.CREDIT_LIMIT";
                ...
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener limite de credito y dias credito: " + err.message });
            }
        );
    }

    obtenerSiTieneDiasDeCreditoVencidos(cliente: Cliente, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT ";
                ...
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener dias de credito vencidos: " + err.message });
            }
        );
    }

    obtenerSaldoActual(cliente: Cliente, configuracionDecimales: ManejoDeDecimales, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT ";
    ...
    obtenerSaldoDeFacturas(cliente: Cliente, configuracionDecimales: ManejoDeDecimales, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT ";
                sql += " IFNULL(SUM(I.TOTAL_AMOUNT),0) TOTAL_AMOUNT";
    ...
    obtenerSaldoDeOrdenesDeVenta(cliente: Cliente, configuracionDecimales: ManejoDeDecimales, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT ";
    ...
    validarDatosGeneralesCuentaCorriente(cliente: Cliente, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        try {
            this.tareaServicio.obtenerRegla('NoValidarAntiguedadDeSaldos', (reglas: Regla[]) => {
                if (reglas.length > 0) {
                    if (reglas[0].enabled === 'Si') {
                        callback(cliente);
                    } else {
                        this.validarAntiguedadDeSaldos(cliente, (clienteN1: Cliente) => {
                            callback(clienteN1);
                        }, (resultado: Operacion) => {
            ...
            callbackError(<Operacion>{ codigo: -1, mensaje: "Error al validar cuenta corriente: " + err.message });
        }
    }

    validarAntiguedadDeSaldos(cliente: Cliente, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        try {
            if (cliente.cuentaCorriente.facturasVencidas) {
                callbackError(<Operacion>{ codigo: -1, mensaje: "Tiene facturas vencidas" });
            } else if (cliente.cuentaCorriente.limiteDeCredito <= 0) {
                callbackError(<Operacion>{ codigo: -1, mensaje: "El cliente no tiene configurado el límite de crédito" });
            } else if (cliente.cuentaCorriente.diasCredito <= 0) {
                callbackError(<Operacion>{ codigo: -1, mensaje: "El cliente no tiene configurado la cantidad de días de crédito" });
            } else if (cliente.cuentaCorriente.diasCreditoVencidos) {
                callbackError(<Operacion>{ codigo: -1, mensaje: "Tiene una factura emitida que ya vencieron los días de crédito" });
            } else {
                callback(cliente);
            }
        } catch (err) {
    ...
    validarCuentaCorriente(cliente: Cliente, listasku: Sku[], ordenDeVentaTipo: any, configuracionDecimales: ManejoDeDecimales, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        try {
            var totalSku = 0;
            ...
            if (ordenDeVentaTipo === OrdenDeVentaTipo.Contado) {
                ...
                if (trunc_number((cliente.cuentaCorriente.saldoActual + cliente.totalAmout + totalSku), configuracionDecimales.defaultCalculationsDecimals) <= trunc_number((cliente.cuentaCorriente.limiteDeCredito), configuracionDecimales.defaultCalculationsDecimals)) {
                    ...
                    my_dialog("", "", "close");
                    callbackError(<Operacion>{ codigo: -1, mensaje: "El crédito es insuficiente" });
                }
            }
    ...
    enviarSolicitudParaObtenerCuentaCorriente(socketIo:SocketIOClient.Socket,cliente: Cliente, opcionValidarSaldoCliente: string, ordenDeVentaTipo: string, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        try {
            var data = {
    ...
    obtenerTodosLosClientesAbordo(criterio:String, callback: (clientes: Cliente[]) => void, callbackError: (resultado: Operacion) => void) {
        try {
    ...
    obtenerEtiquetas(cliente: Cliente, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = " SELECT TC.*";
                ...
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener etiquetas del Cliente: " + err.message });
            }
        );
    }

    obtnerFormatoSqlDeInsertarClienteModificado(cliente: Cliente, sequence: string) {
        let sql =
            `INSERT INTO CUSTOMER_CHANGE (CUSTOMER_CHANGE_ID,
                CODE_CUSTOMER,
                PHONE_CUSTOMER,
                ADRESS_CUSTOMER,
                CONTACT_CUSTOMER,
    ...
    obtnerFormatoSqlDeInsertarEtiquetaDeClienteModificado(cliente: Cliente, etiqueta: Etiqueta, sequence: string) {
    ...
    guardarCambiosDeCliente(cliente: Cliente, callback: (cliente: Cliente) => void, callbackError: (resultado: Operacion) => void) {
        this.obtenerSecuenciaDeCambios(this, (sequence: string, controlador: any) => {
            SONDA_DB_Session.transaction(
                (tx) => {
                    ...
                    callbackError(<Operacion>{ codigo: -1, mensaje: "Error al insertar el los cambios del cliente: " + err.message });
                });
        });
    }

    obtenerSecuenciaDeCambios(controlador: any, callback: (sequence: string, controlador: any) => void) {
        try {
            GetNexSequence("CUSTOMER_CHANGE", (sequence) => {
    ...
    obtenerEtiquetasNoAsociadasAlCliente(cliente: Cliente, callback: (etiquetas: Etiqueta[]) => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                ...
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener etiquetas no asociadas al cliente: " + err.message });
            }
        );
    }
        ...
        try {
            SONDA_DB_Session.transaction(
                (tx) => {
                    ...
                    callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener el codigo de cliente: " + err.message });
                }
            );
