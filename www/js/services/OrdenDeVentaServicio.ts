class OrdenDeVentaServicio implements IOrdenDeVentaServicio {
    clienteServicio = new ClienteServicio();
    tareaServicio = new TareaServcio();

    insertarOrdenDeVenta(ordenDeVenta: OrdenDeVenta, callback: () => void, callbackError: (resultado: Operacion) => void) {
        try {
            let sql: string = "";
            let localIsOnline = gIsOnline;
            SONDA_DB_Session.transaction((tx: SqlTransaction) => {
                sql = "SELECT";
                sql += " C.CLIENT_ID";
                sql += " FROM CLIENTS C";
                sql += ` WHERE C.CLIENT_HH_ID_OLD = '${ordenDeVenta.clientId}'`;
                sql += ` OR C.CLIENT_ID = '${ordenDeVenta.clientId}'`;
                tx.executeSql(sql, [],
                    (txClient: SqlTransaction, resultsClient: SqlResultSet) => {
                        if (resultsClient.rows.length > 0) {
                            ordenDeVenta.clientId = (resultsClient.rows.item(0) as any).CLIENT_ID;

                            sql = "SELECT";
                            sql += " T.TASK_BO_ID";
                            sql += " FROM TASK T";
                            sql += ` WHERE T.TASK_ID = ${ordenDeVenta.taskId}`;
                            txClient.executeSql(sql, [],
                                (txTask: SqlTransaction, resultsTask: SqlResultSet) => {
                                    if (resultsTask.rows.length > 0) {
                                        ordenDeVenta.taskIdBo = (resultsTask.rows.item(0) as any).TASK_BO_ID != null ? (resultsTask.rows.item(0) as any).TASK_BO_ID : 0;

                                        //Sales Order Header Insert Format
                                        sql = this.obtnerFormatoSqlDeInsertarOrdenDeVentaHencabezado(ordenDeVenta);
                                        //console.log(sql);
                                        txTask.executeSql(sql);

                                        //Sales Order Detail Insert Format

                                        ordenDeVenta.ordenDeVentaDetalle.map((salesOrderDetail, index, array) => {
                                            sql = this.obtnerFormatoSqlDeInsertarOrdenDeVentaDetalle(salesOrderDetail, index + 1);
                                            //console.log(sql);
                                            txTask.executeSql(sql);

                                            if (localIsOnline === SiNo.No) {
                                                sql = this.actualizaInventarioPreventa(salesOrderDetail);
                                                if (sql !== "") {
                                                    //console.log(sql);
                                                    txTask.executeSql(sql);
                                                }
                                        ...
                                        if (ordenDeVenta.isParent) {
                                            sql = this.actualizaOrdenReferenciaId(ordenDeVenta);
                                            //console.log(sql);
                                            txTask.executeSql(sql);

                                            this.obtenerTotalDeProductosDeOrdenDeVenta(gtaskid,
                                                txTask,
                                                (qty: number, total: number, transTotal: SqlTransaction) => {
                                                    let html = '<p>';
                                                    ...
                                                    sql = this.obtenerQueryInsertarTareasAuxiliar(gtaskid, html);
                                                    //console.log(sql);
                                                    transTotal.executeSql(sql);
                            ...
                            callbackError(<Operacion>{ codigo: -1, mensaje: "Error al obtener el codigo de cliente: Sin resultados" });
                        }
                    }
                );
            }, (errClient: SqlError) => {
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al insertar la orden de venta: " + errClient.message });
            });
        } catch (e) {
            callbackError(<Operacion>{ codigo: -1, mensaje: "Error al insertar la orden de venta: " + e.message });
        }
    }

    obtnerFormatoSqlDeInsertarOrdenDeVentaHencabezado(ordenDeVenta: OrdenDeVenta) {
        var sql = "";
    ...
    obtnerFormatoSqlDeInsertarOrdenDeVentaDetalle(ordenDeVentaDetalle: OrdenDeVentaDetalle,numeroDeLinea: number) {
    ...
    actualizaOrdenReferenciaId(ordenDeVenta: OrdenDeVenta) {
        let sql = "";
    ...
    obtenerTotalDeProductosDeOrdenDeVenta(taskid: number, tx, callback: (qty: number, total: number, trans: SqlTransaction) => void, errCallBack: (resultado: Operacion) => void) {
        try {
            let sql = "SELECT SUM(D.QTY) QTY ,SUM(D.TOTAL_LINE) TOTAL";
            ...
            tx.executeSql(sql, [],
                (tx: SqlTransaction, results: SqlResultSet) => {
                    if (results.rows.length >= 1) {
                        let ordenDeVenta: any = results.rows.item(0);
                    ...
                    if (err.code !== 0)
                        errCallBack(<Operacion>{ codigo: -1, mensaje: "Error al obtener Toltal de productos: " + err.message });
                }
            );
        } catch (err) {
            errCallBack(<Operacion>{ codigo: -1, mensaje: "Error al obtener Toltal de productos" + err.message });
    ...
    obtenerQueryInsertarTareasAuxiliar(taskId: number, html: string) {
        let sql = "";
    ...
    actualizaInventarioPreventa(ordenDeVentaDetalle: OrdenDeVentaDetalle) {
        let sql = "";
    ...
    obtenerFormatoDeImpresionPreSale(cliente: Cliente, ordenDeVenta: OrdenDeVenta, callback: (formato: string) => void, callbackError: (resultado: Operacion) => void) {
        try {
            ...
            if (ordenDeVenta.discount > 0) {
                let totalConDescuento = (totalDeOrden - ((ordenDeVenta.discount * totalDeOrden) / 100));
            }            
            ...
            //lfooter += "RIGHT 550 T 0 2 0 " + pRow + " Q" + format_number(ordenDeVenta.totalAmount - ordenDeVenta.discountApplied, 2) + "\r\n";
            ...
            if ((ordenDeVenta.discountApplied - ordenDeVenta.totalAmountDisplay) > 0) {
                ...
                //lfooter += "RIGHT 550 T 0 2 0 " + pRow + " Q" + format_number(ordenDeVenta.discountApplied - ordenDeVenta.totalAmountDisplay, 2) + "\r\n";
            }
            ...
            //lfooter += "RIGHT 550 T 0 2 0 " + pRow + " Q" + format_number(ordenDeVenta.discountApplied, 2) + "\r\n";
    ...
    actualizarDocumnetoImpreso(taskId: number, documento: string, documentoDePago: string, callback: () => void, errCallBack: (resultado: Operacion) => void) {
            ...
            (tx) => {
                ...
                tx.executeSql(sql, [],
                    (tx, results) => {
                        ...
                        if (err.code !== 0)
                            errCallBack(<Operacion>{ codigo: -1, mensaje: "Error al actualizar el documento de impreso: " + err.message });
                    }
                );
            },
            (err: SqlError) => {
    ...
    obtenerOrdenDeVentaPorTarea(tarea: Tarea, decimales: ManejoDeDecimales, callback: (ordenDeVenta: OrdenDeVenta) => void, errCallBack: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = "SELECT ";
                ...
                tx.executeSql(sql, [],
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            ...
                            var ordenDeVentaTemp: any = results.rows.item(0);
                            ...
                            this.obtenerOrdenDeVentaDetalle(ordenDeVenta, decimales, (ordenDeVenta: OrdenDeVenta) => {
                                var total = 0;
    ...
    obtenerOrdenDeVentaDetalle(ordenDeVenta: OrdenDeVenta, decimales: ManejoDeDecimales, callback: (ordenDeVenta: OrdenDeVenta) => void, errCallBack: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                let listaDeLi: string[] = [];
                    ...
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            ...
                            for (let i = 0; i < results.rows.length; i++) {
                                const detalleTemp: any = results.rows.item(i);
                                const ordenDeVentaDetalle = new OrdenDeVentaDetalle();
                            ...
                            errCallBack(<Operacion>{ codigo: -1, mensaje: "Error no se encontraron detalles de la orden de venta." });
                        }
                    });
            },
            (err: SqlError) => {
    ...
    obtenerFormatoImpresoOrdenDeVenta(tarea: Tarea, callback: (formato: string) => void, errCallBack: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = " SELECT ";
                sql += " DOC_PRINTED ";
                ...
                tx.executeSql(sql, [],
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            ...
                            errCallBack(<Operacion>{ codigo: -1, mensaje: "Error no se encontro el formato de impresion" });
                        }
                    });
            },
            (err: SqlError) => {
                errCallBack(<Operacion>{ codigo: -1, mensaje: "Error al obtener el formato de impresion: " + err.message });
            }
        );
    }

    obtenerFormatoImpresoOrdenDeVentaPago(tarea: Tarea, callback: (formato: string) => void, errCallBack: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = " SELECT ";
                sql += " PAYMENT_PRINTED";
    ...
    actualizarVecesImpresionOrdenDeVenta(tarea: Tarea, ordenDeVenta: OrdenDeVenta, callback: () => void, errCallBack: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
            ...
            (err: SqlError) => {
                errCallBack(<Operacion>{ codigo: -1, mensaje: "Error no se pudo actualizar la cantidad de veces de impresion.: " + err.message });
            }
        );
    }

    obtenerVecesImpresionOrdenDeVenta(tarea: Tarea, callback: (cantidadDeVencesDeOrdenDeCompa: number, cantidadDeVencesDePago: number) => void, errCallBack: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = " SELECT ";
                ...
                tx.executeSql(sql, [],
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            ...
                            errCallBack(<Operacion>{ codigo: -1, mensaje: "Error no se encontro la cantidad de veces para impresion" });
                        }
                    });
            },
            (err: SqlError) => {
                errCallBack(<Operacion>{ codigo: -1, mensaje: "Error al obtener la cantidad de veces para impresion: " + err.message });
            }
        );
    }

    insertarOrdenDeVentaDraft(ordenDeVenta: OrdenDeVenta, callback: () => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                let sql = this.obtnerFormatoSqlDeInsertarOrdenDeVentaHencabezado(ordenDeVenta);
                tx.executeSql(sql);
                ordenDeVenta.ordenDeVentaDetalle.map((ordenDeVentaDetalle, index, array) => {
                    sql = this.obtnerFormatoSqlDeInsertarOrdenDeVentaDetalle(ordenDeVentaDetalle, index + 1);
                    tx.executeSql(sql);
                });
                callback();
            }, (err: SqlError) => {
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al insertar el borrador de orden de venta: " + err.message });
            });
    }

    actualizarOrdenDeVentaDraft(ordenDeVenta: OrdenDeVenta, callback: () => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = this.obtnerFormatoSqlDeActualizacionOrdenDeVentaHencabezado(ordenDeVenta);
                tx.executeSql(sql);
                sql = this.obtnerFormatoSqlDeBorrarOrdenDeVentaDetalle(ordenDeVenta);
                tx.executeSql(sql);
                ordenDeVenta.ordenDeVentaDetalle.map((ordenDeVentaDetalle, index, array) => {
                    sql = this.obtnerFormatoSqlDeInsertarOrdenDeVentaDetalle(ordenDeVentaDetalle, index + 1);
                    tx.executeSql(sql);
    ...
    obtnerFormatoSqlDeActualizacionOrdenDeVentaHencabezado(ordenDeVenta: OrdenDeVenta) {
        var sql = "";
        sql += " UPDATE SALES_ORDER_HEADER SET ";
        ...
        //sql += " , SALES_ORDER_ID_BO = " + ordenDeVenta.salesOrderIdBo;
        //sql += " , IS_POSTED = " + ordenDeVenta.isPosted;
    ...
    obtnerFormatoSqlDeBorrarOrdenDeVentaDetalle(ordenDeVenta: OrdenDeVenta) {
        var sql = "";
    ...
    obtnerFormatoSqlDeCancelarOCompletarOrdenDeVentaDraft(ordenDeVenta: OrdenDeVenta) {
        var sql = "";
        sql += " UPDATE SALES_ORDER_HEADER SET ";
        sql += " IS_VOID = 1";
        sql += ", IS_UPDATED = 0";
    ...
    cancelarOCompletarOrdenDeVentaDraft(ordenDeVenta: OrdenDeVenta, callback: () => void, callbackError: (resultado: Operacion) => void) {
        SONDA_DB_Session.transaction(
            (tx) => {
                var sql = this.obtnerFormatoSqlDeCancelarOCompletarOrdenDeVentaDraft(ordenDeVenta);
                tx.executeSql(sql);
                callback();
            }, (err: SqlError) => {
                callbackError(<Operacion>{ codigo: -1, mensaje: "Error al insertar el borrador de orden de venta: " + err.message });
    ...
    obtenerCantidadDeSkuPorOrdenDeVenta(ordenDeVenta: OrdenDeVenta, callback: (cantidad: number) => void) {
        let i = 0;
        let item = new OrdenDeVentaDetalle();
        let cantidad = 0;
    ...
    marcarOrdenesDeVentaComoPosteadasYValidadasDesdeBo(ordenesDeVenta: any,callback: () => void,errorCallback: (resultado: Operacion) => void) {
        try {
            SONDA_DB_Session.transaction((tx: SqlTransaction) => {
                ordenesDeVenta.map(ordenDeVenta => {
                    let sql =
                        `UPDATE SALES_ORDER_HEADER SET IS_POSTED = 2, IS_POSTED_VALIDATED = 2 WHERE DOC_SERIE = '${ordenDeVenta.DOC_SERIE}' AND DOC_NUM = ${ordenDeVenta.DOC_NUM}`;
                    tx.executeSql(sql);
                });
            },(err: SqlError) => {
                errorCallback({ codigo: -1, resultado: ResultadoOperacionTipo.Error, mensaje: err.message } as Operacion);
            },callback);
        } catch (e) {
    ...
    ObtenerTotalSinDescuentoDeOrdenDeVenta() : string{
        let listaDeLaCadena: string[] = [];
        listaDeLaCadena.push(" SELECT ");
        ...
        SONDA_DB_Session.transaction(
            (tx) => {                
                ...
                tx.executeSql(sql, [],
                    (tx: SqlTransaction, results: SqlResultSet) => {
                        if (results.rows.length >= 1) {
                            let detalleTemp: any = results.rows.item(0);
                            let total: number = detalleTemp.TOTAL
                            callback(total);
                        } else {
                            callback(0);
                        }
            ...
            (err: SqlError) => {
                errCallBack(<Operacion>{ codigo: -1, mensaje: "Error al obtener el total: " + err.message });
            }
        );
    }
