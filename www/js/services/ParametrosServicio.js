var ParametroServicio = {
        ...
        socketIo.on("send_parameter_from_bo", function (data) {
            switch (data.option) {
                ...
                case "send_parameter_from_bo_not_found":
                    ParametroServicio.MostrarError("No se encontraron parametros para CONSIGNACION, por favor, contacte a su Administrador");
                    break;
                ...
                case "not_found_GetInvoiceParameter":
                    ParametroServicio.MostrarError("No se encontraron parametros para FORMATO DE IMPRESION, por favor, contacte a su Administrador");
                    break;
                case "GetInvoiceParameter_completed":
                    ToastThis("Recepción de parámetros completada...");
                    break;
                case "not_found_GetDeliveryParameter":
                    ParametroServicio.MostrarError("No se encontraron parametros para ENTREGA, por favor, contacte a su Administrador");
                    break;
                case "add_GetDeliveryParameter":
                    ...
                    if (data.inLogin) {
                        ...
                        var invoiceInRoute = localStorage.getItem("INVOICE_IN_ROUTE") == "1";
                        var deliveryInRoute = localStorage.getItem("DELIVERY_IN_ROUTE") == "1";
                        menuControlador.mostrarUOcultarOpcionesDeFacturacionYEntrega(invoiceInRoute, deliveryInRoute, function () { });
                    }
                    break;
        ...
        try {
            if (parametro !== undefined) {
                SONDA_DB_Session.transaction(function (tx) {
                    var sql = "INSERT INTO PARAMETERS(IDENTITY,GROUP_ID,PARAMETER_ID,VALUE) VALUES(" + parametro.IDENTITY + ",'" + parametro.GROUP_ID + "', '" + parametro.PARAMETER_ID + "','" + parametro.VALUE + "')";
                    tx.executeSql(sql);
                }, function (err) {
                    if (err.code !== 0) {
                        notify("No se pudo insertar el parametro debido a: " + err.message);
                    }
                });
            }
        } catch (e) {
    ...
    LimpiarTablaDeParametros: function () {
        try {
            SONDA_DB_Session.transaction(function (tx) {
                ...
                if (err.code !== 0) {
                    notify("No se pudo limpiar la tabla de Parametros debido a: " + err.message);
                }
            });
        } catch (e) {
            notify(e.message);
    ...
    ,
    ObtenerParametro: function (grupoDeParametro, idParametro, callback, errCallback) {
        try {
            SONDA_DB_Session.transaction(
                function (tx) {
                    var sql = "SELECT IDENTITY, GROUP_ID, PARAMETER_ID, VALUE";
                    ...
                    tx.executeSql(sql, [],
                        function (tx, results) {
                            if (results.rows.length > 0) {
                                ...
                                errCallback({ message: "No se ha podido encontrar el parámetro: " + idParametro });
                            }
