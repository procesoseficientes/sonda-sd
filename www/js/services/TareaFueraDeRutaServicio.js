function obtenerClientesFueraDeRutaBD(ruta, filtro) {
  if (gIsOnline == EstaEnLinea.Si) {
    ...
    SocketControlador.socketIo.emit(
      "GetClientsForTaskOutsideTheRoutePlan",
      data
    );
    console.log("GetClientsForTaskOutsideTheRoutePlan");
  } else {
    notify(
      "No se puede establecer comunicación con el servidor. Por favor intente más tarde."
    );
...
function insertarListaDePreciosPorClienteFueraDeRuta(
  data,
  clientCode,
  callBack
) {
  try {
    ...
    SONDA_DB_Session.transaction(
      function(tx) {
        pSql =
          "UPDATE TASK SET CODE_PRICE_LIST = '" +
          data.data.tarea.CODE_PRICE_LIST +
          "' WHERE TASK_ID = " +
          data.data.tarea.TASK_ID;
        tx.executeSql(pSql);
      },
      function(err) {
        InteraccionConUsuarioServicio.desbloquearPantalla();
  ...
  try {
    SONDA_DB_Session.transaction(
      function(tx) {
        ...
        tx.executeSql(
          "DELETE FROM PRICE_LIST_BY_SKU_PACK_SCALE WHERE CODE_PRICE_LIST = '" +
            data.data.tarea.CODE_PRICE_LIST +
            "'"
        );
...
function agregarInformacionDeFacturasVencidas(data, callbak) {
  try {
    ...
    InteraccionConUsuarioServicio.desbloquearPantalla();
    console.log(
      "No se han podido almacenar las facturas vencidas del cliente debido a: " +
        e.message
    );
...
function agregarInformacionDeCuentaCorriente(data, callback) {
  try {
    if (data.cuentaCorriente) {
      var cuentaCorrienteServicio = new CuentaCorrienteServicio();
      cuentaCorrienteServicio.agregarCuentaCorrienteDeCliente(
    ...
    InteraccionConUsuarioServicio.desbloquearPantalla();
    console.log(
      "No se ha podido almacenar la información de cuenta corriente debido a: " +
        e.message
    );
    notify(
      "Error al intentar almacenar la información de cuenta corriente del cliente."
    );
  }
}
