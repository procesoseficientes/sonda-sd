...
function DelegarASincronizacion() {
    ...
    $("#UiBtnEnviarDatos").bind("touchstart", function () {
        ...
        EnviarDatosBajoDemanda(function () {
...
function ActualizarClienteNuevoHandHeld(clienteNuevo, callback, errCallback) {
    ...
    if (_preSalesControllerCustomer != undefined && _preSalesControllerCustomer.CustomerId === delete clienteNuevo.client.CodigoHH) {
    ...
    SONDA_DB_Session.transaction(
        function(tx) {
            ...
            var sql = "UPDATE CLIENTS SET IS_POSTED = 2 " +
                ",CLIENT_ID = '" +
            ...
            sql = "UPDATE CLIENTS_FREQUENCY SET CODE_CUSTOMER = '" +
                clienteNuevo.client.CodigoBo +
                "'  WHERE CODE_CUSTOMER = '" +
                clienteNuevo.client.CodigoHH +
                "'";
            ...
            sql = "UPDATE TAGS_X_CUSTOMER SET CUSTOMER_SYNC = 1, CUSTOMER = '" +
                clienteNuevo.client.CodigoBo +
                "' WHERE CUSTOMER = '" +
                clienteNuevo.client.CodigoHH +
                "'";
            ...
            //ACTUALIZA EN BASE DE DATOS LOCAL LAS TAREAS ASIGNADAS A ESE CLIENTE
                ...
                sql = "UPDATE SALES_ORDER_HEADER SET CLIENT_ID = '" +
                    clienteNuevo.client.CodigoBo +
                    "'  WHERE CLIENT_ID = '" +
                    clienteNuevo.client.CodigoHH +
                    "'";
                ...
                var sqltask = "UPDATE PRESALES_ROUTE SET RELATED_CLIENT_CODE = '" +
                    clienteNuevo.client.CodigoBo +
                ...
                sql = "UPDATE PAYMENT_HEADER SET CLIENT_ID = '" +
                    clienteNuevo.client.CodigoBo +
                    "'  WHERE CLIENT_ID = '" +
                    clienteNuevo.client.CodigoHH +
                    "'";
                ...
                sql = "UPDATE TASK SET RELATED_CLIENT_CODE = '" +
                    clienteNuevo.client.CodigoBo +
                    "'  WHERE RELATED_CLIENT_CODE = '" +
                    clienteNuevo.client.CodigoHH +
                    "'";
...
function ObtenerLineasDePagosNoPosteados(pago, errCallBack, pagos, i, callback, returncallBack) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql = "SELECT * ";
...
function ObtenerDetalleDePagosNoPosteados(callback, errCallBack, pagos) {
    ...
    for (i = 0; i < pagos.length; i++) {
        ObtenerLineasDePagosNoPosteados(pagos[i], errCallBack, pagos, i, function (pagosm, index, pago) {
            pagosm[index] = pago;
            return pagosm;
        }, callback);
    }
...
function EnviarPagos(callback, errCallback) {
    ...
    _enviandoPagos = true;
    var pagos = Array();
    ObtenerPagosNoPosteados(function (pagos) {
        ObtenerDetalleDePagosNoPosteados(function (pagos) {
                 ...
                 function (tx, results) {
                     for (var i = 0; i < results.rows.length ; i++) {
                         var cliente = {
                             ...
                             , SyncId: (results.rows.item(i).SYNC_ID === "null" || results.rows.item(i).SYNC_ID === null || results.rows.item(i).SYNC_ID === "undefined") ? (gCurrentRoute + fecha[0] + fecha[1] + results.rows.item(i).CLIENT_ID) : results.rows.item(i).SYNC_ID
...
function ObtenerLineasDeFrecuencioasNoPosteados(cliente, errCallBack, clientes, i, callback, returncallBack) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql = "SELECT *";
...
function ObtenerFrecuenciaDeClientesNoPosteados(callback, errCallBack, clientes) {
    var i;
    for (i = 0; i < clientes.length; i++) {
        ObtenerLineasDeFrecuencioasNoPosteados(clientes[i], errCallBack, clientes, i, function (clientesN1, index, cliente) {
...
function EnviarClientesNoPosteados(callback, errCallback) {
    ...
    var clientes = Array();
    ObtenerClientessNoPosteados(function (clientesN1) {
        ObtenerFrecuenciaDeClientesNoPosteados(function (clientesN2) {
            for (var i = 0; i < clientesN2.length; i++) {
                ...
                ActualizarEnvioCliente(data, function (pData) {
        ...
        }, errCallback, clientesN1);
    }, errCallback, clientes);
}
...
function ActualizarEtiqutaPorClienteHandHeld(etiquetaPorCliente) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql = "UPDATE TAGS_X_CUSTOMER SET IS_POSTED = 2 WHERE TAG_COLOR = '" + etiquetaPorCliente.tagscolor + "' AND CUSTOMER = '" + etiquetaPorCliente.customer + "'";
            tx.executeSql(sql);
        },
        function (err) {
            if (err.code !== 0)
...
///*********************ENVIO DE TAREAS *********************/
{
    ...
    function EnviarTareas(callback, errCallback) {
        ...
        var tareas = Array();
        ObtenerTareasNoPosteados(function (tareasN1) {
...
{
    function ObtenerOrdernesDeCompraNoPosteados(callback, errCallBack, ordenesDeCompra, usaFiltro) {
        SONDA_DB_Session.transaction(
             function (tx) {
                 var sql = "SELECT DISTINCT *";
    ...
    function ObtenerLineasDeOrdernesDeCompraNoPosteados(ordenDeCompra, errCallBack, ordenesDeCompra, i, callback, returncallBack) {
        SONDA_DB_Session.transaction(
            function (tx) {
                var sql = "SELECT * ";
    ...
    function ObtenerDetalleDeOrdernesDeCompraNoPosteados(callback, errCallBack, ordenesDeCompra) {
        var i;
        for (i = 0; i < ordenesDeCompra.length; i++) {
            ObtenerLineasDeOrdernesDeCompraNoPosteados(ordenesDeCompra[i], errCallBack, ordenesDeCompra, i, function (ordenesDeCompraN1, index, ordenDeCompra) {
    ...
    function EnviarOrdenDeVenta(ordenesdeCompra, index, esUltimo) {
        ...
        setTimeout(function () {
            var actual = index + 1;
            EnviarOrdenDeVenta(ordenesdeCompra, actual, (ordenesdeCompra.length - 1) === actual);
        }, 5000);
    ...
    function EnviarOrdenesDeVenta(callback, errCallback) {
        ...
        ObtenerOrdernesDeCompraNoPosteados(function (ordenesDeCompraN1) {
            ObtenerDetalleDeOrdernesDeCompraNoPosteados(function (ordenesDeCompraN2) {
                if (ordenesDeCompraN2.length > 0)
                    EnviarOrdenDeVenta(ordenesDeCompraN2, 0, (ordenesDeCompraN2.length - 1) === 0);
                _enviandoOrdenesDeCompra = false;
            }, errCallback, ordenesDeCompraN1);
        }, errCallback, ordenesDeCompra, true);
...
{
    function ObtenerNumeroDeImprecionesDeOrdernesDeCompraNoPosteados(callback, errCallBack, ordenesDeCompra) {
        SONDA_DB_Session.transaction(
             function (tx) {
                 var sql = "SELECT *";
                 sql += " FROM SALES_ORDER_HEADER ";
    ...
    function ActualizarEnvioDeNumeroDeImprecionesDeOrdenesDeVenta(data, callback, errCallBack) {
        SONDA_DB_Session.transaction(
            function (tx) {
                var sql = "UPDATE SALES_ORDER_HEADER";
    ...
    function EnviarNumeroDeImprecionesDeOrdenesDeVenta(callback, errCallback) {
        ...
        var ordenesDeCompra = Array();
        ObtenerNumeroDeImprecionesDeOrdernesDeCompraNoPosteados(function (ordenesDeCompraN1) {
...
{
    function ActualizarInventarioReservadoPorOrdenesDeVentaAnuladas(data, estado, callback, errCallBack) {
        SONDA_DB_Session.transaction(
            function (tx) {
    ...
    function ObtenerInventarioReservadoPorOrdenesDeVentaAnuladas(callback, errCallBack, ordenesDeVenta) {
        SONDA_DB_Session.transaction(
             function (tx) {
                 var sql = "SELECT D.SKU, D.QTY, D.SALES_ORDER_ID, D.DOC_NUM, D.DOC_SERIE";
    ...
    function EnviarInventarioReservadoPorOrdenesDeVentaAnuladas() {
        ...
        ObtenerInventarioReservadoPorOrdenesDeVentaAnuladas(function (ordenesDeVentaN1) {
            ...
            ToastThis("Error al devolver el invetario reservado por ordenes de venta anuladas: " + err.message);
        }, ordenesDeVenta);
    }
}
...
function EnviarDataSinClientes() {
    if (gIsOnline) {
        ...
        setTimeout(EnviarNumeroDeImprecionesDeOrdenesDeVenta(function () { }, function (err) { }), 100);

        setTimeout(EnviarNumeroDeSecuenciaDeDocumentos(), 100);

        setTimeout(EnviarInventarioReservadoPorOrdenesDeVentaAnuladas(), 100);

        setTimeout(EnviarBorradoresDeOrdenesDeCompra(function () { }, function (err) { }), 100);

        setTimeout(EnviarActualizacionesDeBorradoresDeOrdenesDeCompra(function () { }, function (err) { }), 100);

        setTimeout(EnviarTomasDeInventario(function () { }, function (err) { }), 100);

        setTimeout(function () { if (_enviandoHistoricoDePromociones) return; enviarHistoricoDePromocionesNoPosteadas(); }, 200);
...
function InsertarNuevaEtiqueta(data, callback, errCallBack) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql = "INSERT INTO TAGS(TAG_COLOR,TAG_VALUE_TEXT,TAG_PRIORITY,TAG_COMMENTS)";
            sql += " VALUES (";
            sql += " '" + data.TagColor + "'";
...
///*********************ENVIO DE BORRADORES DE ORDES DE COMPRA *********************/
{
    ...
    function ObtenerBorradoresDeOrdernesDeCompraNoPosteados(callback, errCallBack, ordenesDeCompra) {
        SONDA_DB_Session.transaction(
             function (tx) {
                 var sql = "SELECT *";
                 sql += " FROM SALES_ORDER_HEADER ";
    ...
    function ObtenerLineasDeBorradoresDeOrdernesDeCompraNoPosteados(ordenDeCompra, errCallBack, ordenesDeCompra, i, callback, returncallBack) {
        SONDA_DB_Session.transaction(
            function (tx) {
                var sql = "SELECT * ";
                sql += " FROM SALES_ORDER_DETAIL ";
    ...
    function ObtenerDetalleDeBorradoresDeOrdernesDeCompraNoPosteados(callback, errCallBack, ordenesDeCompra) {
        var i;
        for (i = 0; i < ordenesDeCompra.length; i++) {
            ObtenerLineasDeBorradoresDeOrdernesDeCompraNoPosteados(ordenesDeCompra[i], errCallBack, ordenesDeCompra, i, function (ordenesDeCompraN1, index, ordenDeCompra) {
                ordenesDeCompraN1[index] = ordenDeCompra;
                return ordenesDeCompraN1;
            }, callback);
        }
    ...
    function ActualizarEnvioDeBorradoresDeOrdernesDeCompra(data, callback, errCallBack) {
        SONDA_DB_Session.transaction(
            function (tx) {
                var sql = "UPDATE SALES_ORDER_HEADER";
                sql += " SET IS_POSTED = 2";
    ...
    function EnviarBorradoresDeOrdenesDeCompra(callback, errCallback) {
        ...
        var ordenesDeCompra = Array();
        ObtenerBorradoresDeOrdernesDeCompraNoPosteados(function (ordenesDeCompraN1) {
            ObtenerDetalleDeBorradoresDeOrdernesDeCompraNoPosteados(function (ordenesDeCompraN2) {
                if (ordenesDeCompraN2.length > 0)
                    EnviarBorradorDeOnrdeDeVenta(ordenesDeCompraN2, 0, (ordenesDeCompraN2.length - 1) === 0);
    ...
    function ObtenerActualizacionesDeBorradoresDeOrdernesDeCompraNoPosteados(callback, errCallBack, ordenesDeCompra) {
        SONDA_DB_Session.transaction(
             function (tx) {
                 var sql = "SELECT *";
                 sql += " FROM SALES_ORDER_HEADER ";
    ...
    function ActualizarEnvioDeActualizacionDeBorradoresDeOrdernesDeCompra(data, callback, errCallBack) {
        SONDA_DB_Session.transaction(
            function (tx) {
                var sql = "UPDATE SALES_ORDER_HEADER";
    ...
    function EnviarActualizacionDeBorradorDeOnrdeDeVenta(ordenesdeCompra, index, esUltimo) {
        var data =
                    {
                        'salesOrder': ordenesdeCompra[index]
                        , 'dbuser': gdbuser
    ...
    function EnviarActualizacionesDeBorradoresDeOrdenesDeCompra(callback, errCallback) {
        ...
        var ordenesDeCompra = Array();
        ObtenerActualizacionesDeBorradoresDeOrdernesDeCompraNoPosteados(function (ordenesDeCompraN1) {
            ObtenerDetalleDeBorradoresDeOrdernesDeCompraNoPosteados(function (ordenesDeCompraN2) {
                if (ordenesDeCompraN2.length > 0)
                    EnviarActualizacionDeBorradorDeOnrdeDeVenta(ordenesDeCompraN2, 0, (ordenesDeCompraN2.length - 1) === 0);
    ...
    function ObtenerTomasDeInventarioNoPosteados(callback, errCallBack, tomasDeInventario) {
        SONDA_DB_Session.transaction(
             function (tx) {
                 var sql = "SELECT *";
    ...
    function ObtenerLineasDeTomasDeInventarioNoPosteados(tomaDeInventario, errCallBack, tomasDeInventario, i, callback, returncallBack) {
        SONDA_DB_Session.transaction(
            function (tx) {
                var sql = "SELECT * ";
                sql += " FROM TAKE_INVENTORY_DETAIL ";
                sql += " WHERE TAKE_INVENTORY_ID=" + tomaDeInventario.TakeInventoryId + "";
                tx.executeSql(sql, [],
                    function (tx, results) {
                        for (var j = 0; j < results.rows.length; j++) {
                        ...
                        tomasDeInventario = callback(tomasDeInventario, i, tomaDeInventario);
    ...
    function ObtenerDetalleDeTomasDeInventarioNoPosteados(callback, errCallBack, tomasDeInventario) {
        var i;
        for (i = 0; i < tomasDeInventario.length; i++) {
            ObtenerLineasDeTomasDeInventarioNoPosteados(tomasDeInventario[i], errCallBack, tomasDeInventario, i, function (tomasDeInventarioN1, index, tomaDeInventario
                ) {
    ...
    function EnviarTomaDeInventario(tomasDeInventario, index, esUltimo) {
        ...
        setTimeout(function () {
            var actual = index + 1;
            EnviarTomaDeInventario(tomasDeInventario, actual, (tomasDeInventario.length - 1) === actual);
        }, 5000);
    }
    ...
    function EnviarTomasDeInventario(callback, errCallback) {
        ...
        ObtenerTomasDeInventarioNoPosteados(function (tomasDeInventarioN1) {
            ObtenerDetalleDeTomasDeInventarioNoPosteados(function (tomasDeInventarioN2) {
                if (tomasDeInventarioN2.length > 0)
                    EnviarTomaDeInventario(tomasDeInventarioN2, 0, (tomasDeInventarioN2.length - 1) === 0);
                _enviandoTomaDeInventario = false;
            }, errCallback, tomasDeInventarioN1);
        }, errCallback, tomasDeInventario);
    ...
    function EnviarCambiosDeClientes(callback, errCallback) {
        ...
        ObtenerCambiosDeClientes(function (clientesN1) {
            ObtenerEtiquetasDeCambiosDeClientes(function (clientesN2) {
                if (clientesN2.length > 0)
                    EnviarCambioCliente(clientesN2, 0, (clientesN2.length - 1) === 0);
                _enviandoCambiosDeClientes = false;
            }, errCallback, clientesN1);
        }, errCallback, cambiosDeClientes);
    }
    ...
    function ObtenerEtiquetasDeCambiosDeClientes(callback, errCallback, clientes) {
        var i;
        for (i = 0; i < clientes.length; i++) {
    ...
    function ObtenerTagsDeClientesNoPosteados(cliente, errCallBack, clientes, i, callback, returncallBack) {
    ...
    function ObtenerLineasDeFrecuencioasDeTodosLosScouting(cliente, errCallBack, clientes, i, callback, returncallBack) {
        SONDA_DB_Session.transaction(
            function (tx) {
                var sql = "SELECT *";
                sql += " FROM CLIENTS_FREQUENCY";
    ...
    function ObtenerFrecuenciaDeTodosLosScouting(callback, errCallBack, clientes) {
        var i;
        for (i = 0; i < clientes.length; i++) {
            ObtenerLineasDeFrecuencioasDeTodosLosScouting(clientes[i], errCallBack, clientes, i, function (clientesN1, index, cliente) {
                clientesN1[index] = cliente;
                return clientesN1;
            }, callback);
        }
    ...
    function EnviarDatosBajoDemanda(callback, errCallback) {
        if (socket) {
            EnviarTodasLasOrdenesDeVenta(function () {
                EnviarTodosLosScouting(function () {
                    EnviarTodasEtiquetas(function () {
                        EnviarBorradoresDeBonificaciones(OrigenDeEnvioDeBorradoresDeBonificacion.ReportarDispositivo);
                        callback();
                    }, function (err) {
                        errCallback(err);
                    });
    ...
    function EnviarTodasLasOrdenesDeVenta(callback, errCallback) {
        ...
        }, errCallback, ordenesDeCompra, false);
    }

    function EnviarTodosLosScouting(callback, errCallback) {
        var clientes = [];
        ObtenerTodosLosScouitng(function (clientesN1) {
            ObtenerFrecuenciaDeTodosLosScouting(function (clientesN2) {
    ...
    function EnviarBorradoresDeBonificaciones(origenDeEnvio) {
        ...
        bonoServicio.obtenerBorradoresDeBonificacion(function (borradores) {
        ...
        SONDA_DB_Session.readTransaction(function (tx) {
            ...
            tx.executeSql(sql.join(""),
                ...
                function (txResult, error) {
                    if (error.code !== SqliteError.Desconocido) {
                        errorCallback("No se ha podido obtener el histórico de las promociones debido a: " +
                            error.message);
                    }
                });
        },
            function (err) {
                if (err.code != SqliteError.Desconocido) {
                    errorCallback("No se ha podido obtener el histórico de las promociones debido a: " + err.message);
                }
            });
    }
    ...
    function MarcarHistoricoDePromocionesComoPosteado(historicoDePromociones) {
        SONDA_DB_Session.transaction(function (tx) {
            historicoDePromociones.map(function (promo) {
                tx.executeSql("UPDATE HISTORY_BY_PROMO SET IS_POSTED = 2, SERVER_POSTED_DATETIME = '" + promo.SERVER_POSTED_DATETIME + "' WHERE DOC_SERIE = '" + promo.DOC_SERIE + "' AND DOC_NUM = " + promo.DOC_NUM);
            });
        },
            function (err) {
                if (err.code !== SqliteError.Desconocido) {
                    notify("No se ha podido actualizar el histórico de las promociones debido a: " + err.message);
                }
            });
    }
    ...
    function enviarHistoricoDePromocionesNoPosteadas() {
        ...
        obtenerHistoricoDePromocionesNoPosteadas(function (historialDePromociones) {
