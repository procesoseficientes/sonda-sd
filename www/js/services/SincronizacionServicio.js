...
function EnviarData() {
    try {
        if (gIsOnline) {
            ...
            setTimeout(EnviarValidacionDeClientes(function () { }, function (err) { }), 100);

            setTimeout(EnviarFacturas(function () { }, function (err) { }), 100);

            setTimeout(EnviarConsignaciones(function () { }, function (err) { }), 100);

            setTimeout(EnviarConsignacionesPagadas(function () { }, function (err) { }), 100);

            setTimeout(EnviarDocumentosDeDevolucionDeInventarioRecogidoDeConsignacion(function () { }, function () { }), 100);

            setTimeout(EnviarRegistroDeTrazabilidadDeConsignacion(function () { }, function () { }), 100);

            setTimeout(EvniarTareas(function () { }, function (error) { }), 100);

            setTimeout(EnviarConsignacionesAnuladas(function () { }, function (error) { }), 100);

            setTimeout(EnviarEncuestasDeCompraDeCompetencia(function () { }, function (error) { }), 100);

            setTimeout(EnviarDepositosNoPosteados(function () { }, function () { }), 100);

            setTimeout(EnviarResolucion("SendInvoice"), 100);

            setTimeout(EnviarValidacionDeFactura(function () { }, function (err) { }), 200);

            setTimeout(ObtenerBroadcastPerdidos(), 2000);

            setTimeout(EnviarNumeroDeSecuenciaDeDocumentos(), 100);

            setTimeout(EnviarFacturasConNumeroDeTelefonoAsociado(function () {},function () {}),100);
...
{
    function ObtenerNumeroDeSecuenciaDeDocumentos(callback, errCallBack, documentos) {
    ...
    function EnviarNumeroDeSecuenciaDeDocumentos() {
        ...
        ObtenerNumeroDeSecuenciaDeDocumentos(function (documentosN1) {
            ...
            ToastThis("Error al actualizar secuencia de documentos: " + err.message);
        }, documentos);
    }
}
...
function EnviarConsignaciones(callback, errCallback) {
    ...
    ObtenerConsignacionesNoPosteadas(function (consignacionesN1) {
        ObtenerDetalleDeConsignaciones(function (consignacionesN2) {
...
function ObtenerConsignacionesNoPosteadas(callback, errCallback, consignaciones) {
    SONDA_DB_Session.transaction(
         function (tx) {
             var sql = "SELECT *";
...
function ObtenerDetalleDeConsignacionesNoPosteados(consignacion, errCallBack, consignaciones, i, callback, returncallBack) {
...
function EnviarConsignacionesPagadas(callback, errCallback) {
    ...
    var consignaciones = Array();
    ObtenerConsignacionesPagadasNoPosteadas(function (consignacionesN1) {
        ...
        notify("Error al ObtenerConsignacionesPagadasNoPosteadas " + error.mensaje);
    }, consignaciones);
}

function ObtenerConsignacionesPagadasNoPosteadas(callback, errCallback, consignaciones) {
    SONDA_DB_Session.transaction(
         function (tx) {
             var sql = "SELECT * ";
             sql += " FROM CONSIGNMENT_HEADER ";
...
function ObtenerDetalleDeConsignacionesPagadas(callback, errCallback, consignaciones) {
    var i;
    for (i = 0; i < consignaciones.length; i++) {
...
function ObtenerDetalleDeConsignacionesPagadasNoPosteados(consignacion, errCallBack, consignaciones, i, callback, returncallBack) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql = "SELECT *  ";
            sql += " FROM CONSIGNMENT_DETAIL ";
...
function ActualizarConsignaciones(consignaciones) {
    try {
        ...
        SONDA_DB_Session.transaction(
            ...
            function (err) {
                ToastThis("No se pudo actualizar la informacion de la consignacion: " + consignacion.CONSIGNMENT_ID.toString() + " debido a: " + err.message);
            },
            function () {
        ...
        notify("No se pudo actulizar la informacion de la consignacion enviada desde el servidor debido a: " + e.message);
    }
}
...
function EnviarDocumentosDeDevolucionDeInventarioRecogidoDeConsignacion(callBack, errorCallBack) {
    ...
    ObtenerDocumentosDeDevolucionDeInventarioNoPosteados(function (documentos) {
        ObtenerDetalleDeDocumentoDeDevolucion(function (documentosCompletos) {
            if (documentosCompletos.length > 0)
                EnviarDocumentosDeDevolucionDeInventario(documentosCompletos, 0, (documentosCompletos.length - 1) === 0);
...
function ObtenerDocumentosDeDevolucionDeInventarioNoPosteados(callBack, errorCallBack, documentosDeDevolucion) {
    try {
        ...
        errorCallBack("Error al intentar obtener los Documentos de Devolución de Inventario Recogido De Consignación debido a: " + e.message);
    }
}

function ObtenerDetalleDeDocumentoDeDevolucion(callBack, errorCallBack, documentos) {
    var i;
...
function ObtenerDetalleDeDevolucionesNoPosteadas(documento, errorCallBack, documentos, indice, callBack, returnCallBack) {
    SONDA_DB_Session.transaction(
           function (tx) {
               var sql = "SELECT " +
                   "SKU_COLLECTED_ID" +
                   ...
                   " FROM SKU_COLLECTED_DETAIL WHERE SKU_COLLECTED_ID =" + parseInt(documento.SKU_COLLECTED_ID);
...
function EnviarDocumentosDeDevolucionDeInventario(documentos, index, esUltimo) {
    ...
    setTimeout(function () {
        var actual = index + 1;
        EnviarDocumentosDeDevolucionDeInventario(documentos, actual, (documentos.length - 1) === actual);
    }, 5000);
}
...
function EnviarRegistroDeTrazabilidadDeConsignacion(callBack, errorCallBack) {
    ...
    var registros = Array();
    ObtenerRegistrosDeTrazabilidadDeConsignacionNoPosteados(function (registrosDeTrazabilidad) {
        if (registrosDeTrazabilidad.length > 0) {
            var intervaloDeEnvioDeRegistroDeTrazabilidadDeConsignacion = setInterval(function () {
                ...
                if (registrosDeTrazabilidad.length === 0) {
                    registros = null;
                    clearInterval(intervaloDeEnvioDeRegistroDeTrazabilidadDeConsignacion);
    ...
    }, errorCallBack, registros);
}

function ObtenerRegistrosDeTrazabilidadDeConsignacionNoPosteados(callBack, errorCallBack, registrosDeTrazabilidad) {
...
function EnviarRegistroDeTrazabilidad(registrosDeTrazabilidad, index, esUltimo) {
    var data =
                {
    ...
    setTimeout(function () {
        var actual = index + 1;
        EnviarRegistroDeTrazabilidad(registrosDeTrazabilidad, actual, (registrosDeTrazabilidad.length - 1) === actual);
    }, 5000);
}
...
function EvniarTareas(callBack, errorCallBack) {
    ...
    }, errorCallBack, tareas);
}

function ObtenerTareasNoPosteadas(callBack, errorCallBack, tareas) {
    SONDA_DB_Session.transaction(function (trans) {
        var sql = "SELECT TASK_ID, RELATED_CLIENT_CODE, TASK_BO_ID, COMPLETED_SUCCESSFULLY, REASON, ACCEPTED_STAMP, COMPLETED_STAMP, TASK_STATUS, POSTED_GPS, TASK_SEQ FROM TASK " +
            "WHERE IS_POSTED = 2";
        trans.executeSql(sql, [],
            function (trans2, results) {
                for (var i = 0; i < results.rows.length; i++) {
                    var tarea = {
                        ...
                        , completedSuccessfully: results.rows.item(i).COMPLETED_SUCCESSFULLY
                ...
                _enviandoTareas = false;
                if (error.code !== 0) {
                    errorCallBack("Error al obtener las tareas no posteadas debido a: " + error.message);
                }
            });
    }, function (error) {
        _enviandoTareas = false;
...
function MarcarTareaComoSincronizada(tareas, errorCallBack) {
    SONDA_DB_Session.transaction(function (trans) {
        tareas.map(function (tarea) {
            var sql = "UPDATE TASK SET IS_POSTED = 3 WHERE TASK_ID = " + parseInt(tarea.TASK_ID) + " AND TASK_BO_ID = " + parseInt(tarea.TASK_BO_ID);
            trans.executeSql(sql);
        });
    }, function (error) {
        if (error.code !== 0) {
            errorCallBack("No se pudo actualizar la tarea sincronizada debido a: " + error.message);
        }
    });
}
...
function EnviarConsignacionesAnuladas(callBack, errorCallBack) {
    ...
    ObtenerConsignacionesAnuladasNoPosteadas(function (consignacionesAnuladasNoPosteadas) {
        if (consignacionesAnuladasNoPosteadas.length > 0)
            EnviarConsignacionAnulada(consignacionesAnuladasNoPosteadas, 0, (consignacionesAnuladasNoPosteadas.length - 1) === 0);
        _enviandoConsignacionesAnuladas = false;
    }, errorCallBack, consignacionesAnuladas);
}

function ObtenerConsignacionesAnuladasNoPosteadas(callback, errCallback, consignaciones) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql = "SELECT *";
            sql += " FROM CONSIGNMENT_HEADER ";
...
function EnviarConsignacionAnulada(consignaciones, index, esUltimo) {
    ...
    setTimeout(function () {
        var actual = index + 1;
        EnviarConsignacionAnulada(consignaciones, actual, (consignaciones.length - 1) === actual);
    }, 5000);
}

function MarcarConsignacionAnuladaComoSincronizada(consignacion, errorCallBack) {
    SONDA_DB_Session.transaction(function (trans) {
        var sql = "UPDATE CONSIGNMENT_HEADER SET IS_POSTED = 6 WHERE CONSIGNMENT_ID = " + parseInt(consignacion.ConsignmentId) + " AND DOC_NUM = " + parseInt(consignacion.DocNum);
        trans.executeSql(sql);
    }, function (error) {
        if (error.code !== 0) {
            errorCallBack("No se pudo actualizar la consignación anulada debido a: " + error.message);
        }
    });
}
...
function EnviarEncuestasDeCompraDeCompetencia() {
    try {
        EncuestaServicio.ObtenerEncuestasDeCompraDeCompetenciaNoEnviadas(function (encuestas) {
        ...
        notify("Error al intentar enviar las encuestas de compra de competencia: " + e.message);
    }
...
function EnviarDepositosNoPosteados(callBack, errorCallBack) {
    try {
        ...
        if (_estaEnviandoDepositos) {
            ...
            ObtenerDepositosNoPosteados(function (depositos) {
    ...
    try {
        ...
        SONDA_DB_Session.transaction(function (tx) {
            ...
            tx.executeSql(sql, [], function (txResult, results) {
                ...
                if (error.code !== 0) {
                    errorCallBack("No se pudo obtener la lista de Depositos debido a: " + error.message);
                }
            });
        }, function (err) {
            if (err.code !== 0) {
                errorCallBack("No se pudo inicar la transaccion debido a: " + err.message);
            }
        });
    } catch (e) {
        errorCallBack("Error al obtener los depositos no posteados debido a: " + e.message);
    }
}
...
function ObtenerFacturasNoPosteadas(callback, errCallBack, facturas) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql =
                "SELECT * FROM INVOICE_HEADER WHERE IS_CREDIT_NOTE = 0 AND IS_POSTED IN (0,1) AND STATUS <> 3 ORDER BY INVOICE_NUM";
                ...
                function (tx, results) {
                    for (var i = 0; i < results.rows.length; i++) {
                        var factura = {
                            ...
                            DueDate: results.rows.item(i).DUE_DATE ? results.rows.item(i).DUE_DATE : null,
                            CreditAmount: results.rows.item(i).CREDIT_AMOUNT ? results.rows.item(i).CREDIT_AMOUNT : 0,
                            CashAmount: results.rows.item(i).CASH_AMOUNT ? results.rows.item(i).CASH_AMOUNT : 0,
                            PaidToDate: results.rows.item(i).PAID_TO_DATE ? results.rows.item(i).PAID_TO_DATE : 0,
                            TaskId: results.rows.item(i).TASK_ID,
...
function ObtenerLineasDeFacturaNoPosteados(factura, errCallBack, facturas, i, callback, returncallBack) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql = "SELECT * ";
...
function ObtenerDetalleDeFacturaNoPosteados(callback, errCallBack, facturas) {
    ...
    for (i = 0; i < facturas.length; i++) {
        ObtenerLineasDeFacturaNoPosteados(facturas[i], errCallBack, facturas, i, function (facturasN1, index, factura) {
...
function EnviarFactura(facturas, index, esUltimo) {
    ...
    setTimeout(function () {
        var actual = index + 1;
        EnviarFactura(facturas, actual, (facturas.length - 1) === actual);
    }, 5000);
}
...
function EnviarFacturas(callback, errCallback) {
    ...
    var facturas = Array();
    ObtenerFacturasNoPosteadas(function (facturasN1) {
        ObtenerDetalleDeFacturaNoPosteados(function (facturasN2) {
            if (facturasN2.length > 0)
                EnviarFactura(facturasN2, 0, (facturasN2.length - 1) === 0);
            _enviandoFacturas = false;
        }, errCallback, facturasN1);
    }, errCallback, facturas, true);
...
function EnviarValidacionDeFactura(callback, errCallback) {
    ...
    VerificarDocumentosEnServidor(TipoDeValidacionDeFactura.EnRuta, false, function () {
...
function VerificarDocumentosEnServidor(tipoDeValidacionDeFactura, obtenerTodasLasFacturas, callback, errCallback) {
    if (gIsOnline == EstaEnLinea.No) {
        errCallback("No hay conexion hacia el servidor");
        return;
    }

    ObtenerFacturasSincronizadas(obtenerTodasLasFacturas, function (facturas) {
        FormarEnvioDeFacturas(facturas, function (enviarFacturas) {
            if (tipoDeValidacionDeFactura === TipoDeValidacionDeFactura.FinDeRuta) {
...
function ObtenerFacturasSincronizadas(obtenerTodas, callback, errCallback) {
    SONDA_DB_Session.transaction(
        function (tx) {
            var sql = "SELECT H.INVOICE_NUM, H.ID_BO, COUNT(D.SKU) AS DETAIL_NUM, MAX(H.CLIENT_ID) CLIENT_ID, " +
            ...
            sql += " WHERE H.IS_CREDIT_NOTE = 0 AND H.IS_POSTED =" + EstadoEnvioDoc.EnviadoConAcuseDeRecibido;
            if (!obtenerTodas) {
    ...
    try {
        ...
        for (var i = 0; i < facturas.rows.length; i++) {
            var factura = {
                ...
                DetailNum: facturas.rows.item(i).DETAIL_NUM === null ||
...
function ActualizarEstadoDeFactura(invoiceId, estado, estadoDeValidacion, index, callback, errCallback) {
    try {
        SONDA_DB_Session.transaction(
            function (tx) {
                var sql = "UPDATE INVOICE_HEADER";
...
function CambiarEstadoParaReenviarFacturas(facturas, callback, errCallback) {
    try {
        for (var i = 0; i < facturas.length; i++) {
            ActualizarEstadoDeFactura(facturas[i].DOC_NUM, (facturas[i].RESULT === EstadoEnvioDoc.NoEnviado ? EstadoEnvioDoc.NoEnviado : EstadoEnvioDoc.EnviadoConAcuseDeRecibido), (facturas[i].RESULT === EstadoDeValidacionDeOrdenDeVenta.PendienteDeValidar ? EstadoDeValidacionDeOrdenDeVenta.PendienteDeValidar : EstadoDeValidacionDeOrdenDeVenta.EnviadoConAcuseDeValidado), i, function (index) {
            ...
            tx.executeSql(sql, [],
                function (tx, results) {
                    if (results.rows.length < 1) {
                        ...
                        errCallBack("No se han validado todas las facturas, espere por favor.");
                    }
                },
                function (tx, err) {
                    if (err.code !== 0)
...
function EnviarValidacionDeClientes(callback, errCallback) {
    ...
    VerificarClientesEnServidor(false, TipoDeValidacionDeCliente.EnRuta, function () {
...
function EnviarScouting() {
    ...
    clienteServicio.obtenerClientesConEtiquetasNoSincronizados(function (clientes) {
        ...
        notify("No se han podido obtener los clientes no sincronizados debido a: " + resultado.mensaje);
    });
}
            ...
            tx.executeSql(sql, [],
                function (txReturn, results) {
                    if (results.rows.length < 1) {
                        ...
                        errCallBack("No se han validado todos los clientes, espere por favor.");
                    }
                },
...
function VerificarClientesEnServidor(obtenerTodosLosClientes, tipoDeValidacionDeCliente, callback, errCallback) {
    if (gIsOnline == EstaEnLinea.No) {
        errCallback("No hay conexion hacia el servidor");
        return;
    }
    ...
    var clienteServicio = new ClienteServicio();
    clienteServicio.obtenerClientesParaValidacionEnBo(obtenerTodosLosClientes,
        function (clientes) {
            if (clientes.length > 0) {
                if (tipoDeValidacionDeCliente === TipoDeValidacionDeCliente.FinDeRuta) {
                    SocketControlador.socketIo.on("ValidateScoutingsPOS_Request" + tipoDeValidacionDeCliente, callback);
                }
                var data = {
                    'scouting': clientes,
                    'dbuser': gdbuser,
...
function EnviarFacturasConNumeroDeTelefonoAsociado(callback, errCallback) {
    ...
    var facturas = [];
    ObtenerFacturasConNumeroTelefonico(function (facturasN1) {
...
function EnviarNotasDeEntrega() {
    ...
    notaDeEntregaServicio.obtenerDocumentosParaSincronizacion(function (documentosParaSincronizar) {
        if (documentosParaSincronizar.length > 0) {
            var intervaloDeEnvioDeNotasDeEntrega = setInterval(function () {
                ...
                SocketControlador.socketIo.emit("InsertDeliveryNotesFromSondaSd", data);
                if (documentosParaSincronizar.length === 0) {
...
function EnviarEntregasCanceladas() {
    ...
    entregaServicio.obtenerEntregasCanceladasParaSincronizacion(function (entregasCanceladasParaSincronizar) {
        if (entregasCanceladasParaSincronizar.length > 0) {
            var intervaloDeEnvioDeEntregasCanceladas = setInterval(function () {
                entregasCanceladasAEnviar = entregasCanceladasParaSincronizar.splice(0, 5);//Se envian los clientes de 5 en 5
                var data = {
                ...
                SocketControlador.socketIo.emit("InsertCanceledDeliveryFromSondaSd", data);
                if (entregasCanceladasParaSincronizar.length === 0) {
...
function EnviarDemandasDeDespachoPorTarea() {
    ...
    entregaServicio.obtenerDemandasDeDespachoPorTareaParaSincronizacion(function (demandasDeDespachoPorTareaParaSincronizar) {
        if (demandasDeDespachoPorTareaParaSincronizar.length > 0) {
            var intervaloDeEnvioDeDemandasDeDespachoPorTarea = setInterval(function () {
                demandasDeDespachoPorTareaAEnviar = demandasDeDespachoPorTareaParaSincronizar.splice(0, 5);//Se envian los clientes de 5 en 5
                var data = {
                ...
                SocketControlador.socketIo.emit("InsertPickingDemandByTaskFromSondaSd", data);
                if (demandasDeDespachoPorTareaParaSincronizar.length === 0) {
...
function MarcarManifiestos3PlComoCompletos() {
    ...
    manifiestoServicio.obtenerManifiestos3PlParaSincronizacion(function (manifiestos3PlParaSincronizar) {
        if (manifiestos3PlParaSincronizar.length > 0) {
            this.EnviarManifiesto3pl(manifiestos3PlParaSincronizar, 0, (manifiestos3PlParaSincronizar.length - 1) === 0);
...
function EnviarManifiesto3pl(manifiestos, index, esUltimo) {
    ...
    manifiestoControlador.cambiarEstadoManifiesto3plEnElServidor(manifiestos[index].manifestHeaderId, EstadoDeManifiesto.Completado.toString());
    if (esUltimo) return;
    setTimeout(function () {
        var actual = index + 1;
...
function EnviarNotasDeEntregaAnuladas() {
    ...
    notaDeEntregaServicio.obtenerNotasDeEntregaAnuladasParaSincronizacion(function (documentosParaSincronizar) {
        if (documentosParaSincronizar.length > 0) {
            var intervaloDeEnvioDeNotasDeEntrega = setInterval(function () {
                ...
                SocketControlador.socketIo.emit("InsertDeliveryNoteCanceledFromSondaSd", data);
                if (documentosParaSincronizar.length === 0) {
                    notasDeEntregaAEnviar = null;
                    clearInterval(intervaloDeEnvioDeNotasDeEntrega);
...

{
    function EnviarDocumentosDePagoDeFacturasVencidas() {
        ...
        pagoServicio.obtenerDocumentosDePagoNoPosteadosEnElServidor(function(documentosNoPosteados) {
