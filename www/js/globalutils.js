...
function fakescann() {
    ...
    globalUtilsServicio.socketIo.emit('manifest_scanned', { 'scanned': gManifestID });
...
function DeviceIsOnline() {
    ...
    try {
        ...
        tipoDeRedALaQueEstaConectadoElDispositivo = gNetworkState.toString() === "0" ? "[]": states[gNetworkState];
        ...
        if (gLoginStatus === "OPEN") {
            if (!socket || !socket.connected) {
                var validacionDeLicencia = new ValidacionDeLicenciaControlador(new Messenger());
                validacionDeLicencia.validarLicencia(localStorage.getItem("pUserID"),
        ...
        //    notify("El tipo de conexión a internet cambió.");
        //    $.unblockUI();
        ...
        alert("DeviceIsOnline: " + e.message);
...
function onBackKeyDown() {
    ...
    switch ($.mobile.activePage[0].id) {
        ...
        case "page_new_client":
            var newClientPanel = $.mobile.activePage.children('[id="new_client_panel"] ');
            var clientPanel = $.mobile.activePage.children('[id="client_panel"] ');

            if (newClientPanel.css("visibility") === "hidden" && clientPanel.css("visibility") === "hidden") {
                UsuarioDeseaCancelarClienteNuevo();
            } else {
                if (newClientPanel.css("visibility") !== "hidden")
                    newClientPanel.panel("toggle");
                else
                    clientPanel.panel("toggle");
            }
                ...
                tx.executeSql(pSQL, [],
                    function (tx, results) {
                        if (results.rows.length >= 1) {
                            $("#btnFinishCollecting").removeClass("ui-disabled").addClass("ui-enabled");
                        } else {
                            $("#btnFinishCollecting").removeClass("ui-enabled").addClass("ui-disabled");
                        }
                    },
                    function (tx, err) {
                        my_dialog("", "", "close");
                        alert("ShowHideOptions: " + err.message);
                    }
                );
            },
            function (err) {
        ...
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                tx.executeSql(pSQL, [],
                    function (tx, results) {
                        if (results.rows.length >= 1) {
                            $("#btnFinishDelivery").removeClass("ui-disabled").addClass("ui-enabled");

                        } else {
                            $("#btnFinishDelivery").removeClass("ui-enabled").addClass("ui-disabled");
                        }
                    },
                    function (err) {
                        my_dialog("", "", "close");
            ...
            function (err) {
                alert("ShowHideOptions.3.Error processing SQL: " + err.message);
            }
        );
    }
    catch (e) {
...
function gettask(taskid) {
    try {
        SONDA_DB_Session.transaction(
                ...
                tx.executeSql(pSQL, [],
                    function (tx, results) {
                        ...
                        if (results.rows.length >= 1) {
                            ...
                            $("#lblClientName_pickup").text(results.rows.item(0).RELATED_CLIENT_NAME);
                            $("#lblAddress_pickup").text(results.rows.item(0).TASK_ADDRESS);
                            //$("#lblAddress_directions").val(results.rows.item(0).TASK_COMMENTS);

                            $("#btnMyPickupRoutePhone").text(results.rows.item(0).RELATED_CLIENT_PHONE_1);
                            $("#btnMyPickupRoutePhone").unbind("touchstart");
                            $("#btnMyPickupRoutePhone").bind("touchstart", function () { MakeACall(results.rows.item(0).RELATED_CLIENT_PHONE_1); });
                            ...
                            $("#btnTakeMeThere").bind("touchstart", function () { navigateto(); });
                                ...
                                case TareaEstado.Completada:
                                    ...
                                    switch (gTaskType) {
                                        ...
                                        case TareaTipo.Preventa:
                                            ...
                                            tareaServicio.obtenerTarea(tarea, function (tareaN1) {
                                                if (tareaN1.completedSuccessfully) {
                                                    ...
                                                    $.mobile.changePage("#UiPageTaskCompletedWithReason", {
                                                        transition: "flow",
                                                        reverse: true,
                                                        changeHash: false,
                                                        showLoadMsg: false
                    ...
                    function (err) {
                        my_dialog("", "", "close");
                        if (err.code !== 0) {
                            alert("(gettask.0)Error processing SQL: " + err.code);
                        }
                    }
                );
            },
            function (err) {
                if (err.code !== 0) {
                    alert("(gettask.1)Error processing SQL: " + err.code);
                }
            }
        );
...
function irAOrdenDeVentaValidandoSaldoDeCliente() {
    ValidarSaldoCliente(gClientID, 0, "", 0, OpcionValidarSaldoCliente.EjecutarTarea, gSalesOrderType, function () {
        SeguirTareaPreventa();
    }, function (err) {
        notify(err.message);
    });
...
function RefreshMyRoutePlan() {
    ...
    try {
        ...
        function (tx) {
            ...
            tx.executeSql('SELECT * FROM PRESALES_ROUTE', [],
                function (tx, results) {
                    ...
                    for (i = 0; i <= (results.rows.length - 1) ; i++) {
                        ...
                        vLI = '';
                        vLI = '<li style="opacity: 1" class="ui-alt-icon ui-nodisc-icon"> <a href="#" onclick=' + pClick + '>';

                        var pDocNum = "";
                        if (results.rows.item(i).DOC_NUM === null || results.rows.item(i).DOC_NUM === 0) {
                            if (results.rows.item(i).TASK_TYPE === "DELIVERY") {
                                pDocNum = "Fuera de ruta";
                            } else if (results.rows.item(i).TASK_TYPE === "TAKE_INVENTORY") {
                        ...
                        switch (results.rows.item(i).TASK_TYPE) {
                            case TareaTipo.Entrega:
                                break;
                            default:
                                vLI += '<span lass="ui-li small-roboto">' + pDocNum + '</span></p>';
                                break;
                        }

                        vLI += '<p><span class="small-roboto">' + gtaskid + ") " + results.rows.item(i).RELATED_CLIENT_CODE + ' ' + results.rows.item(i).RELATED_CLIENT_NAME + '</span></p>';
                        vLI += '<p><span class="small-roboto">' + results.rows.item(i).TASK_ADDRESS + '</span></p>';

                        switch (results.rows.item(i).TASK_STATUS) {
                            case TareaEstado.Asignada:
                                vLI += '<span class="ui-li-count small-roboto" style="background-color:yellow"><img src="css/styles/images/icons-png/alert-black.png"></span>';
                                break;
                            case TareaEstado.Aceptada:
                                vLI += '<span class="ui-li-count small-roboto" style="background-color:lime"><img src="css/styles/images/icons-png/check-black.png"></span>';
                                break;
                            case TareaEstado.Completada:
                                vLI += '<span class="ui-li-count small-roboto" style="background-color:lightsteelblue"><img src="css/styles/images/icons-png/tag-black.png"></span>';
                                break;
                            default:
                                vLI += '<span class="ui-li-count ui-btn small-roboto" style="background-color:silver">Sin Status</span>';
                                break;
                        };

                        switch (results.rows.item(i).NO_PICKEDUP) {
                            case 1:
                                vLI += '<p><span class="small-roboto" style="text-shadow: none; color:#ffffff; background-color:orangered">No Recolectado. ' + results.rows.item(i).NO_VISIT_REASON + '</span></p>';
                                break;
                            case 0:
                                vLI += '<p><span class="small-roboto" style="background-color:silver"></span></p>';
                                break;
                            default:
                                vLI += '<p><span class="small-roboto" style="background-color:silver"></span></p>';
                                break;
                ...
                function (err) {
                    my_dialog("", "", "close");
                    if (err.code !== 0) {
                        alert("(7)Error processing SQL: " + err.code);
                    }
                }
            );
        },
            function (err) {
                if (err.code !== 0) {
                    alert("(100)Error processing SQL: " + err.code);
                }
            }
        );
    }
        ...
        if (pskus_count >= 1) {
            ...
            SONDA_DB_Session.transaction(
                function (tx) {
                    ...
                    pSQL = "INSERT INTO ORDERS(ORDER_ID, CREATED_DATESTAMP, DELIVERY_ADDRESS, DELIVERY_BRANCH_PDE, DELIVERY_BRANCH_NAME, ";
                    pSQL += " DELIVERY_BRANCH_ADDRESS, STATUS, SOURCE_TASK, IS_OFFLINE, TOTAL_AMOUNT, CLIENT_CODE, CLIENT_NAME)";
                    pSQL += " VALUES('" + pNewGuideID + "'," + "'" + xdate + "','" + pClientPickup + "','" + pDeliveryBranchPDE + "','" + pDeliveryBranchName + "','" + pDeliveryBranchAddress + "',";
                    pSQL += " 'CREATED'," + gtaskid + ", 1, " + pTotalAmount + ",'" + gClientID + "', '" + pClientPickup + " ')";
                    ...
                    pSQL = "UPDATE SKUS_X_ORDER SET ORDER_ID = '" + pNewGuideID + "' WHERE ORDER_ID = '-9999'";
                    console.log(pSQL);
                    tx.executeSql(pSQL);
    ...
    try {
        ...
        SONDA_DB_Session.transaction(
           function (tx) {
               ...
               pSQL = "INSERT INTO SKUS_X_ORDER(ORDER_ID, SKU_ID, SKU_DESCRIPTION, QTY, UNIT_PRICE, TOTAL_PRICE, SOURCE_TASK, IS_OFFLINE)";
               pSQL += " VALUES('-9999','" + um_code + "','" + um_desc + "', 1, " + list_price + ", " + list_price + "," + gtaskid + ", 1)";
               tx.executeSql(pSQL);
    ...
    try {
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                var pSQL = "SELECT ORDER_ID, DELIVERY_BRANCH_ADDRESS, DELIVERY_BRANCH_NAME, TOTAL_AMOUNT FROM ORDERS WHERE SOURCE_TASK=" + gtaskid;
                console.log("RefreshMyGuidesOnTask: " + pSQL);
                tx.executeSql(pSQL, [],
                    function (tx, results) {

                        $('#guides_listing').children().remove('li');
                        ...
                        for (i = 0; i <= (results.rows.length - 1) ; i++) {
                            ...
                            var xclick = "get_guide_options('" + results.rows.item(i).ORDER_ID + "')";

                            vLI = '';
                            vLI = '<li class="ui-nodisc-icon ui-noboxshadow ui-icon-alt" onclick="' + xclick + '">';
                            vLI += '<p><span class="small-roboto"><strong>' + results.rows.item(i).ORDER_ID + '</strong>';
                            vLI += '<p><span class="small-roboto">' + results.rows.item(i).DELIVERY_BRANCH_NAME + '</span></p>';
                            vLI += '<p><span class="small-roboto">' + results.rows.item(i).DELIVERY_BRANCH_ADDRESS + '</span></p>';
                            vLI += '<p><span class="ui-li-count small-roboto">Q ' + format_number(results.rows.item(i).TOTAL_AMOUNT, 2) + '</span></p>';
                        ...
                        $("#btnOrderSumm").text("Q " + format_number(parseFloat(pLocalTotal), 2));

                        my_dialog("", "", "close");
                    },
                    function (tx, err) {
                        my_dialog("", "", "close");
                        alert("(21)Error processing SQL: " + err.message);
            ...
            function (err) {
                if (err.code !== 0) {
                    alert("(22)Error processing SQL: " + err.code);
                }
            }
        ...
        alert("RefreshMyGuidesOnTask:" + e.message);
    }
}
    ...
    try {
        ...
        SONDA_DB_Session.transaction(
            function (tx) {
                var pDoc = '';
                var pImg = '';
                var pSQL = "SELECT A.*, B.RELATED_CLIENT_CODE, B.RELATED_CLIENT_NAME FROM ORDERS A, PRESALES_ROUTE B WHERE A.ORDER_ID = '" + pGuide + "' AND B.TASK_ID = A.SOURCE_TASK";
                console.log(pSQL);
                ...
                tx.executeSql(pSQL, [],
                        ...
                        if (results.rows.length >= 1) {
                            while (i <= (results.rows.item(0).LABELS - 1)) {
                                try {
                                    ...
                                    lheader += "M0A," + pGuide + "-" + (i + 1) + "/" + results.rows.item(0).PACKAGES + "\r\n";
                                    lheader += "ENDQR \r\n";
                                    lheader += "LEFT 5 T 4 4 0 70 " + (i + 1) + "/" + results.rows.item(0).PACKAGES + "\r\n";
                                    lheader += "L 5 240 570 240 1\r\n";
                                    lheader += "CENTER 570 T 0 3 0 270 GUIA: " + pGuide + "\r\n";
                                    lheader += "LEFT 5 T 0 2 0 300 REMITENTE    : " + results.rows.item(0).RELATED_CLIENT_CODE + " " + results.rows.item(0).RELATED_CLIENT_NAME + "\r\n";
                                    lheader += "LEFT 5 T 0 2 0 330 DESTINATARIO : " + results.rows.item(0).DESTINATION_CLIENTNAME + "\r\n";
                                    lheader += "LEFT 5 T 0 2 0 360 " + results.rows.item(0).DELIVERY_BRANCH_NAME + " " + results.rows.item(0).DELIVERY_BRANCH_ADDRESS + "\r\n";
                                    lheader += "LEFT 5 T 0 2 0 390 COURIER      : " + gUserCode + " " + gLoggedUser + "\r\n";
                                ...
                                bluetoothSerial.write(lheader, function () {
                                }, function () {
                    ...
                    function (txt, err) {
                        my_dialog("", "", "close");
                        alert("(2)Error processing SQL: " + err.message);
    ...
    try {
        ...
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                pSQL = "UPDATE SKUS_X_ORDER SET QTY = " + pqty + " WHERE ORDER_ID = '-9999' AND SKU_ID = '" + um_code + "'";
                console.log(pSQL);

                tx.executeSql(pSQL);

            },
            function (tx, err) { my_dialog("", "", "close"); notify("um.add.row:" + err); },
            function () {
                RefreshSkusXOrder();
            }
        );
    ...
    try {
        ...
        SONDA_DB_Session.transaction(
           function (tx) {
               ...
               pSQL = "DELETE FROM SKUS_X_ORDER WHERE ORDER_ID = '-9999' AND SKU_ID = '" + um_code + "'";
               console.log(pSQL);
...
function RefreshSkusXOrder() {
    ...
    try {
        ...
        function (tx) {
            ...
            tx.executeSql("SELECT * FROM SKUS_X_ORDER WHERE ORDER_ID = '-9999'", [],
                function (tx, results) {

                    $('#listview_packs_detail').children().remove('li');
                    $("#lblTotalOrder").text("0.00");

                    for (i = 0; i <= (results.rows.length - 1) ; i++) {


                        var pClickRemove = "removepackage('" + results.rows.item(i).SKU_ID + "');";
                        var pClickUpdate = "updatepackage('" + results.rows.item(i).SKU_ID + "');";

                        var pKeyUp = "qtykeyup('" + results.rows.item(i).SKU_ID + "');";

                        vLI = '';
                        vLI = '<li data-corners="false" data-mini="true" class="ui-alt-icon ui-nodisc-icon ui-btn ui-shadow ui-btn-icon-tag "><a href="#">';

                        vLI += '<p><span class="small-roboto">' + results.rows.item(i).SKU_DESCRIPTION + '</span></p>';

                        vLI += '<p><div data-role="controlgroup" data-type="horizontal">';
                        vLI += '<input style="width:35%" class="ui-btn-a ui-corner-all small-roboto allownumericwithoutdecimal" data-corners="true" onblur="' + pKeyUp + '" type="text" id="pack_qty_' + results.rows.item(i).SKU_ID.replace(" ", "") + '" value="' + results.rows.item(i).QTY + '">';

                        vLI += '<span class="small-roboto"> P/U Q ' + format_number(results.rows.item(i).UNIT_PRICE, 2) + '</span></p>';
                        vLI += '<span class="small-roboto ui-li-count">Q' + format_number(parseFloat(results.rows.item(i).UNIT_PRICE) * parseFloat(results.rows.item(i).QTY), 2) + '</span>';
                        vLI += '</div></a>';
                        ...
                        pRunningTotal = parseFloat(pRunningTotal) + (parseFloat(results.rows.item(i).UNIT_PRICE) * parseFloat(results.rows.item(i).QTY));
                ...
                function (err) {
                    my_dialog("", "", "close");
                    if (err.code !== 0) {
                        alert("(4)Error processing SQL: " + err.code);
                    }
                }
            );
        },
    ...
    try {
        ...
        SONDA_DB_Session.transaction(
            function (tx) {
                var pDoc = '';
                var pImg = '';
                $('#manifest_guides_listview').children().remove('li');


                var pSQL = "SELECT * FROM MANIFEST_DETAIL WHERE INVOICE_ID = '" + gGuideToDeliver + "'";
                console.log(pSQL);
                ...
                tx.executeSql(pSQL, [],
                    function (tx, results) {
                        ...
                        gDESTINATION_CLIENTNAME_ToDeliver = results.rows.item(0).DESTINATION_CLIENTNAME + ". " + results.rows.item(0).DESTINATION_ADDRESS;
...
function showmanifestlist(pstatus) {
    ...
    SONDA_DB_Session.transaction(
        function (tx) {
            ...
            var pSQL = "SELECT * FROM MANIFEST_DETAIL WHERE INVOICE_STATUS = '" + pstatus + "'";
            ...
            tx.executeSql(pSQL, [],
                    ...
                    for (i = 0; i <= (results.rows.length - 1) ; i++) {
                        ...
                        switch (results.rows.item(i).GUIDE_STATUS) {
                            case "PENDING":
                                ...
                                vLIStat += '<span class="small-roboto" style="background-color:yellow"><img src="css/styles/images/icons-png/alert-black.png"></span>';
                                break;
                            case "ACCEPTED":
                                //pClick = "getguide(" + results.rows.item(i).GUIDE_ID + ");";
                                pClick = "EjecutarTareaDeVenta(" + results.rows.item(i).RELATED_CLIENT_CODE + ")";
                                vLIStat += '<span class="small-roboto" style="background-color:lime"><img src="css/styles/images/icons-png/check-black.png"></span>';
                                break;
                            case "DELIVERED":
                                pClick = 'notify("Pedido ya fue entregado");';
                                vLIStat += '<span class="small-roboto" style="background-color:lightsteelblue"><img src="css/styles/images/icons-png/tag-black.png"></span>';
                                break;
                            default:
                                vLIStat += '<span class="small-roboto" style="background-color:silver">' + results.rows.item(i).GUIDE_STATUS + '</span>';
                                break;
                        };

                        vLI = '<li class="ui-alt-icon ui-nodisc-icon ui-btn ui-shadow ui-btn-icon-tag"> <a href="#" onclick=' + pClick + '>';
                        vLI += '<p><span class="small-roboto">' + vLIStat + results.rows.item(i).GUIDE_SEQ + ") " + results.rows.item(i).GUIDE_ID + ' ' + results.rows.item(i).DESTINATION_CLIENTNAME + '</span></p>';
                        vLI += '<p><textarea cols="25" rows="2" class="small-roboto">' + results.rows.item(i).DESTINATION_ADDRESS + '</textarea></p>';
                        vLI += '<p><span class="ui-li-count">' + results.rows.item(i).PACKAGES + '</span></p>';
                        vLI += '</a></li>';
                ...
                function (tx, err) {
                    my_dialog("", "", "close");
                    if (err.code !== 0) {
                        alert("(5)Error processing SQL: " + err.message);
                    }
                }
            );
        },
...
function welcome_back() {
    ...
    try {
        ...
        cordova.plugins.notification.badge.configure({ title: '%d Nueva(s) tareas' });
        ...
        //    if (!socket.connected) {
        //        pDebug = '3';
        //        socket = window.io.connect(SondaServerURL);
        ...
        console.log("get my route plan on globalutils");
        ...
        if (estadoDeLaRuta !== "OPEN") {
            console.log("se va a ir por MostrarPaginaDeInicioDeRuta");
            ...
            console.log("se va a ir por menu_page");
            $.mobile.changePage("#menu_page",
                {
                    transition: "flow",
                    reverse: true,
        ...
        function (buttonIndex) {
            if (buttonIndex === 2) {
                ...
                SONDA_DB_Session.transaction(
                    ...
                    function (tx, err) { my_dialog("", "", "close"); notify("finishdeliveryroute.CATCH:" + err); },
                    function () {
                        navigator.app.exitApp();
                    }
                );
...
function filtermydispatchplan(pstatus) {
    try {
        ...
        my_dialog("", "", "close"); notify("filtermy.catch:" + e.message); console.log(e.message);
    }
}
function filtermyplan(pstatus) {
    try {
        ...
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                tx.executeSql("SELECT * FROM PRESALES_ROUTE WHERE TASK_STATUS = '" + pstatus + "' ORDER BY TASK_SEQ", [],
                    function (tx, results) {
                        ...
                        for (i = 0; i <= (results.rows.length - 1) ; i++) {
                            ...
                            //vLI = '<li class="ui-alt-icon ui-nodisc-icon ui-btn ui-shadow ui-btn-icon-tag"> <a href="#" onclick='+ pClick +'>';
                            vLI += '<p><span class="small-roboto">' + (i + 1) + ") " + results.rows.item(i).RELATED_CLIENT_CODE + ' ' + results.rows.item(i).RELATED_CLIENT_NAME + '</span></p>';
                            vLI += '<p><span class="small-roboto">' + results.rows.item(i).TASK_ADDRESS + '</span></p>';
                            ...
                            switch (results.rows.item(i).TASK_STATUS) {
                                ...
                                case TareaEstado.Completada:
                                    ...
                                    break;
                                default:
                                    vLI += '<span class="ui-li-count ui-btn small-roboto" style="background-color:silver">Sin Status ' + results.rows.item(i).TASK_STATUS + '</span>';
                                    break;
                            };
    ...
    catch (e) { my_dialog("", "", "close"); notify("filterplan:" + e.message); console.log(e.message); }
}
...
function format_number(pnumber, decimals) {
    ...
    var result = parseFloat((pnumber * 100) / 100).toFixed(decimals);
    return result;
...
function printtest(pPrinterAddress, pType) {
    try {
        ...
        lheader += "LEFT 5 T 0 2 0 300 REMITENTE    : CODIGO Y NOMBRE REMITENTE\r\n";
        lheader += "LEFT 5 T 0 2 0 330 DESTINATARIO : CODIGO Y NOMBRE DESTINATARIO\r\n";
        ...
        try {
            ...
            bluetoothSerial.isConnected(
                    ...
                    try {
                        ...
                        bluetoothSerial.write(lheader, function () {
                            DesconectarImpresora(
                                ...
                                function() {
                                    alert('Unable to disconnect from printer');
                                    return false;
                                }
                            );
                        }, function () {
                ...
                function () {
                    ...
                    toast_this("Connected to " + pPrinterAddress);
                }
            );

        } catch (e) {
            alert('print.guide.catch:' + e.message);
        }
        my_dialog("", "", "close");


    } catch (e) { notify("print.test.catch:" + e.message); }
...
function onBatteryLow(info) {
    ...
    $(".batteryclass").text(gBatteryLevel + "%");
    $('.batteryclass').buttonMarkup({ icon: "alert" });
    $(".batteryclass").css("color", "white");
    ...
    try {
        ...
        if (gPrepared === 0) {
            try {
                ...
                alert("onDeviceReady:" + ex.message);
            }
        }
        ...
        var resumenOrdenDeVentaControlador = new ResumenOrdenDeVentaControlador(mensajero);
        ...
        var dividirOrdenDeVentaControlador = new DividirOrdenDeVentaControlador(mensajero);
        dividirOrdenDeVentaControlador.delegadoDividirOrdenDeVentaControlador();

        /*var listaDeTareasControlador = new ListaDeTareasControlador(mensajero);
        listaDeTareasControlador.delegadoListaDeTareasControlador();
        listaDeTareasControlador.generarListaTareas();*/


        var tomaDeInventarioControlador = new TomaDeInventarioControlador(mensajero);
        tomaDeInventarioControlador.delegarTomaDeInventarioControlador();

        var unidadesDeMedidaTomaDeInventarioControlador = new UnidadesDeMedidaTomaDeInventarioControlador(mensajero);
        unidadesDeMedidaTomaDeInventarioControlador.delegarUnidadesDeMedidaTomaDeInventarioControlador();

        var listaSkuTomaDeInventarioControlador = new ListaSkuTomaDeInventarioControlador(mensajero);
        listaSkuTomaDeInventarioControlador.delegadoListaSkuTomaDeInventarioControlador();

        var resumenTomaDeInventarioControlador = new ResumenTomaDeInventarioControlador(mensajero);
        ...
        var cambiosEnClienteControlador = new CambiosEnClienteControlador(mensajero);
        cambiosEnClienteControlador.delegarCambiosEnClienteControlador();

        /*var gf = new GeoFences();
        gf.deviceReady();*/

        var impresionManifiestoControlador = new ImpresionManifiestoControlador(mensajero);
        ...
        var bonificacionPorComboControlador = new BonificacionPorComboControlador(mensajero);
        bonificacionPorComboControlador.delegarBonificacionPorComboControlador();

        var validacionDeLicenciaControlador = new ValidacionDeLicenciaControlador(mensajero);
        validacionDeLicenciaControlador.delegarValidacionDeLicenciaControlador();

        var seleccionDeImpresoraControlador = new SeleccionDeImpresoraControlador(mensajero);
        ...
        $("#login_panel").css({ opacity: 0.90 });
        $("#menu_panel").css({ opacity: 0.90 });
        $("#presales_panel").css({ opacity: 0.90 });
        ...
        window.bluetoothSerial.connectInsecure(pPrinterAddress,
            function () {
                $(".printerclass").buttonMarkup({ icon: "print" });
            },
            function () {
        ...
        if (localStorage.getItem("POS_STATUS") !== undefined && localStorage.getItem("POS_STATUS") !== null && localStorage.getItem("POS_STATUS") === "OPEN") {
            if (localStorage.getItem("SeCargaronListas") !== undefined && localStorage.getItem("SeCargaronListas") !== null && localStorage.getItem("SeCargaronListas") === "SI") {
                cargarListaDeTareas();
            }
        }
...
function checkloginstatus() {
    ...
    console.log("gloginstatus:" + gLoginStatus);
...
function simulate_scanpackage() {
    ...
    if (parseInt(xscanned) >= parseInt(xlabels)) {
        ...
        $("#btnDeliveryPhotoSignature").removeClass("ui-disabled").addClass("ui-disabled");
...
function delegate_events() {
    ...
    $("#btnPictureOnDelivery").bind("touchstart", function () { take_picture_pickup('delivery'); });
    $("#btnAcceptOnDelivery").bind("touchstart", function () { CompletedDeliveryTask(); });


    $("#btnDeliveryRoute").bind("touchstart", function () { StartDeliveryRoute(); });

    $(".loggedclass").bind("touchstart", function () {

        var usuario = localStorage.getItem("LAST_LOGIN_NAME") + "\n\r" + localStorage.getItem("pUserID");
        notify(usuario);
        usuario = null;
    });

    $("#btnStartMyManifest").bind("touchstart",
        function () {
    ...
    $("#btnCancelManifest").bind("touchstart",
        function () {
            $("#lblManifest").text("");
    ...
    $("#btnScannManifest").bind("touchstart",
        function () {
                    ...
                    cordova.plugins.barcodeScanner.scan(
                        function (result) {
                            if (result.text.length > 0) {
                                ...
                                if (resultado.indexOf("L") !== -1 && resultado.indexOf("P") !== -1) {
                                    existe = true;
                                }

                                if (existe) {
                                    resultado = result.text.split("LP")[1];
                                    document.getElementById("UiTxtNumeroDeManifiesto").value = resultado;
                            ...
                            cordova.plugins.barcodeScanner.scan(
                                function (result) {
                                    if (result.text.length > 0) {
                                        ...
                                        if (existe) {
                                            resultado = result.text.split("LP")[1];
                                            document.getElementById("UiTxtNumeroDeManifiesto")
                                                .value = resultado;
                                            $.mobile.changePage("#UiPagePrintManifest",
                                            {
                                                transition: "flow",
    ...
    $("#btnDeliveryPhotoSignature").bind("touchstart", function () { SignAndPhoto('delivery'); });

    $("#btnDeliveryPrint").bind("touchstart", function () { printdelivery(); });

    $("#btnDeliveryFinish").bind("touchstart",
        function () {
            navigator.notification.confirm(
    ...
    $("#btnNewTaskOffPlan").bind("touchstart",
        function () {
    ...
    $("#btnSaveTaskOffPlan").bind("touchstart",
        function () {
            create_task_offplan();
        }
    );
    ...
    $("#btnAcceptManifest").bind("touchstart",
        function () {
            navigator.notification.confirm(
    ...
    $("#btntestqrscanner").bind("touchstart",
        function () {
            cordova.plugins.barcodeScanner.scan(
                function (result) {
    ...
    $("#btnFinishCollecting").bind("touchstart",
        function () {
            finishroute();
        }
    );
    $("#btnSignatureGuide").bind("touchstart", function () { SignAndPhoto('pickup'); });
    $("#btnSignatureGuideInvoice").bind("touchstart", function () { SignAndPhoto('pickup'); });

    $("#btnClearSignature").bind("touchstart", function () { clearsignaturepad(); });
    $("#btnClearSignature_delivery").bind("touchstart", function () { clearsignaturepad(); });


    $("#btnTakePicture").bind("touchstart", function () { take_picture_pickup('pickup'); });
    $("#btnTakePicture_delivery").bind("touchstart", function () { take_picture_pickup('delivery'); });


    $("#btnSaveSignAndPicture").bind("touchstart", function () { savesignature('pickup'); });
    $("#btnSaveSignAndPicture_delivery").bind("touchstart", function () { savesignature('delivery'); });


    $("#btnFinishPickup").bind("touchstart", function () { CompletePreSaleTask(); });


    $("#btnGuideIsCompleted").bind("touchstart", function () { GuideCompleted(); });

    //$("#btnAcceptThisTask").bind("touchstart", function () { AcceptTask(); });

    $("#btnNewOrder").bind("touchstart", function () { ShowNewOrder(); });

    $("#btnShowPrinterConfig").bind("touchstart",
        function () {
            if ((localStorage.getItem("isPrinterZebra") === "1")) {
                $.mobile.changePage("#UiPaginaSeleccionDeImpresora",
                    { transition: "flow", reverse: true, changeHash: true, showLoadMsg: false });
            } else {
                exploredevices();
                $.mobile.changePage("#printers_page", { transition: "flow", reverse: true, changeHash: true, showLoadMsg: false });
            }
        }
    );


    $(".printerclass_btn").bind("touchstart",
        function () {
            if ((localStorage.getItem("isPrinterZebra") === "1")) {
                $.mobile.changePage("#UiPaginaSeleccionDeImpresora",
                    { transition: "flow", reverse: true, changeHash: true, showLoadMsg: false });
    ...
    $("#btnTryPrinter1").bind("touchstart",
        function () {
    ...
    $("#btnSavePrinter1").bind("touchstart",
            function () {
                SavePrinter();
            });
    ...
    //$("#pageManifestHeader").on("swiperight", function () {
    //    var panel = $("#UiManifestPanel");
    //    panel.panel("open");
    //    panel = null;
    //});

    //$("#UiBotonImprimirManifiesto").on("click", function() {
    //    $.mobile.changePage("#UiPagePrintManifest", {
    //        transition: "flow",
    //        reverse: true,
    //        changeHash: false,
    ...
    $(document).on("pageshow", "#NotDeliveryReason_page", function () {
        SONDA_DB_Session.transaction(
           function (tx) {
               pSQL = "SELECT * FROM REASONS WHERE REASON_TYPE = 'NOT_DELIVERY_REASONS'";
               gNotDeliveryReasons = [];
               var xclick = '';
               ...
               tx.executeSql(pSQL, [],
                   function (tx, results) {
                       if (results.rows.length >= 1) {
                           ...
                           for (i = 0; i <= (results.rows.length - 1) ; i++) {
                               xclick = "ProcessNotDelivered('" + results.rows.item(i).REASON_PROMPT + "');";
                               //ojo

                               var pInjectedHtml = '<li class="ui-alt-icon ui-nodisc-icon"><a href="#" onclick="' + xclick + '">' + results.rows.item(i).REASON_VALUE + "</a></li>"
           ...
           function (err) {
               alert("ProcessNotDelivered.1.Error processing SQL: " + err.message);
           }
       );
    });
    ...
    $(document).on("pageshow", "#deliver_guides_page", function () {
        try {
            ...
            if (gSignatedDelivery === true) {
                $("#btnDeliveryPrint").removeClass("ui-disabled").addClass("ui-enabled");
            }
            else {
                $("#btnDeliveryPrint").removeClass("ui-disabled").addClass("ui-disabled");
            }
        } catch (e) {
            notify(e.message);
        }
    ...
    $(document).on("pageshow", "#taskpickup_page", function () {
        $('#listview_guidedetail').listview().listview('refresh');
...
function showVoidOptions(pInvoiceID) {
    SONDA_DB_Session.transaction(
        ...
        function (tx, err) {
            alert("(13)Error processing SQL: " + err.message);
        }
    );
    ...
    try {
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                var psql = "SELECT * FROM BRANCHES where CLIENT_CODE = '" + gClientID + "'";
                console.log(psql);
                                ...
                                SONDA_DB_Session.transaction(
                                    function (tx) {
                                        ...
                                        if (item === "NINGUNA") {
                                            ...
                                            var psql1 = "SELECT * FROM BRANCHES where BRANCH_CODE = '" + item + "'";
                                            console.log(psql1);
                                            tx.executeSql(psql1, [],
                                                function (tx, results) {

                                                    $("#btnBranchName").text(results.rows.item(0).BRANCH_NAME);
                                                    $("#btnBranchID").text(results.rows.item(0).BRANCH_CODE);
                                                    $("#lblBranchAddress").text(results.rows.item(0).BRANCH_ADDRESS);
                                                ...
                                                function (err) {
                                                    ...
                                                    my_dialog("", "", "close");
                                                    if (err.code !== 0) {
                                                        alert("(15)Error processing SQL: " + err.code);
                                                    }
                                                }
                                            );
                                        }
                                    ...
                                    function (err) {
                                        if (err.code !== 0) {
                                            alert("(16)Error processing SQL: " + err.code);
                                        }
                                    }
                                );
    ...
    try {
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                var psql = "SELECT * FROM SKUS WHERE SKU_ID NOT IN (SELECT SKU_ID FROM SKUS_X_ORDER WHERE ORDER_ID = '-9999') ";
                console.log(psql);
                tx.executeSql(psql, [],
                    function (tx, results) {
                                ...
                                gBranches.push(
                                    {
                                        ...
                                        text: results.rows.item(i).SKU_DESCRIPTION + "\n Q" + format_number(results.rows.item(i).PRICE_LIST, 2),
...
function GetNoVisitDesc(item) {
    try {
        SONDA_DB_Session.transaction(
                ...
                tx.executeSql(pSQL, [],
                    function (tx, results) {
                        ...
                        if (results.rows.length >= 1) {
                            ...
                            socket.emit("process_novisit", { "taskid": gtaskid, "reason": results.rows.item(0).PARAM_NAME });
                    ...
                    function (err) {
                        my_dialog("", "", "close");
                        if (err.code !== 0) {
                            alert("(GetNoVisitDesc.0)Error processing SQL: " + err.code);
                            return 'ERROR';
                        }
                    }
                );
            ...
            function (err) {
                if (err.code !== 0) {
                    alert("(GetNoVisitDesc.1)Error processing SQL: " + err.code);
                }
            }
        );
    } catch (e) { notify("GetNoVisitDesc: " + e.message); }
}
...
function GetUMDesc(pUM) {
    try {
        SONDA_DB_Session.transaction(
                   ...
                   tx.executeSql(pSQL, [],
                       function (tx, results) {
                           ...
                           if (results.rows.length >= 1) {
                               ...
                               setpackage(pUM, results.rows.item(0).SKU_DESCRIPTION, results.rows.item(0).PRICE_LIST);
                       ...
                       function (tx, err) {
                           ...
                           alert("(GetUMDesc.0)Error processing SQL: " + err.message);
                           return 'ERROR';
               ...
               function (err) {
                   if (err.code !== 0) {
                       alert("(GetUMDesc.1)Error processing SQL: " + err.code);
                   }
               }
           );
    } catch (e) { notify("GetUMDesc: " + e.message); }
}
...
function AcceptTask() {
    ...
    try {
        ...
        if (gIsOnline === 1) {
            var conn_option = { 'taskid': gtaskid, 'dbuser': gdbuser, 'dbuserpass': gdbuserpass, 'gps': "" };
        ...
        switch (gTaskType) {
            case TareaTipo.Entrega:
                ...
                break;
            case TareaTipo.Preventa:
                ValidarSaldoCliente(gClientID, 0, "", 0, OpcionValidarSaldoCliente.EjecutarTarea, OrdenDeVentaTipo.Credito, function () {
                    SeguirTareaPreventa();
                }, function (err) {
                    notify(err.message);
                });
    ...
    try {
        if (impresora !== "") {
            ...
            bluetoothSerial.isConnected(function () {
                ...
                bluetoothSerial.connectInsecure(gPrintAddress,
                    function () {
                        $(".printerclass").buttonMarkup({ icon: "print" });
        ...
        function(result)  {
            if (result != "") //yay printer ready!!!            
                ...
                if (currentTry < 20)
                    ...
                    reject(LanzarExcepcionDesconexion(new Error("Tiempo agotado")));
            }
        },
        function(e){
        ...
        SONDA_DB_Session.transaction(
            function (tx) {
                ...
                if (pIsCheckingOffline === 0) {
                    //SIGNATURE, IMAGE
                    pSQL = "UPDATE ORDERS SET SIGNATURE = '" + pSignature + "', TAKEN_IMAGE = '" + gpicture + "' WHERE SOURCE_TASK = " + gtaskid;
                    console.log(pSQL);

                    tx.executeSql(pSQL);
                }
            },
            function (tx, err) { my_dialog("", "", "close"); notify("ProcessPresaleTask.catch:" + err.message); },
            function () {
                console.log(".....");
            }
        );
                    ...
                    function (tx, results) {
                        ...
                        SONDA_DB_Session.transaction(
                            function (tx) {
                                ...
                                pSQL = 'SELECT * FROM SKUS_X_ORDER WHERE SOURCE_TASK = ' + gtaskid;
                                //console.clear();
                                //console.log(pSQL);
                                ...
                                tx.executeSql(pSQL, [],
                                    function (tx, results) {
                                        ...
                                        var SKUS_X_ORDER = {};
                                        var x = 0;
                                        console.log("results.SKUS_X_ORDER.length:" + results.rows.length);
                                    ...
                                    function (tx, err) {
                                        ...
                                        my_dialog("", "", "close");
                                        alert("post_presale: " + err.message);
    ...
    try {
        if (gIsOnline === 1) {
            ...
            navigator.notification.confirm(
                ...
                function (buttonIndex) {
                    if (buttonIndex === 2) {
                        var conn_option = { "courierid": gLoggedUser, "gps": gCurrentGPS, "batt": gBatteryLevel, 'dbuser': gdbuser, 'dbuserpass': gdbuserpass };
...
function ProcessAcceptManifest() {
    try {
        if (gIsOnline === 1) {
            ...
            notify("ERROR, Necesita estar conectado al servidor, para aceptar el manifiesto");
        }
...
function printdelivery() {
    try {
        ...
        lheader += "ENDQR \r\n";
        lheader += "L 5 240 570 240 1\r\n";
        lheader += "CENTER 570 T 0 3 0 270 ENTREGA DE GUIA: " + gGuideToDeliver + "\r\n";
        lheader += "LEFT 5 T 0 2 0 300 EMPAQUES     : " + gPACKAGES_ToDeliver + "\r\n";
        lheader += "LEFT 5 T 0 2 0 330 REMITENTE    : " + gRELATED_CLIENT_NAME_ToDeliver + "\r\n";
        lheader += "LEFT 5 T 0 2 0 360 DESTINATARIO : " + gDESTINATION_CLIENTNAME_ToDeliver + "\r\n";
        lheader += "LEFT 5 T 0 2 0 420 COURIER      : " + gUserCode + " " + gLoggedUser + "\r\n";
        ...
        try {
            bluetoothSerial.isConnected(
                ...
                function () {
                    ...
                    toast_this("Connected to " + gPrintAddress);
                }
            );
